...
9. **Зона `money` показывается — внутри копилка на мечту, без переводов.**
   ...
   (ADR-0008, отменил решение 2 из ADR-0004). Скрыто адресно (`комната/ключ`)
   восемь зон: предмета нет в интерьере, раскрыть нечем (ADR-0006). Итого
   продукт показывает **122 зоны из 130**.
