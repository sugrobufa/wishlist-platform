...
9. **Зона `money` в продукте не показывается** — за ней нет ни экрана, ни
   ...
   Снимается решением владельца, не кодом (ADR-0004). Плюс восемь зон скрыты
   адресно (`комната/ключ`): предмета нет в интерьере, раскрыть нечем
   (ADR-0006). Итого продукт показывает **112 зон из 130**.
