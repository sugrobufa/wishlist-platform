...
describe("кому достаётся", () => {
  it("гость — с дрейфом", () => {
    // Хвост окна расширен (тикет 96): после `drift` в теге появились свет и
    // время суток комнаты. Замок остался прежним — `drift` внутри тега.
    expect(guestPage).toMatch(/<SceneStage[\s\S]{0,300}?\bdrift\b[\s\S]{0,400}?\/>/u);
  });
  ...
  it("хозяйка — тоже с дрейфом (тикет 103)", () => {
    ...
    expect(ownerPage).toMatch(/<SceneStage[\s\S]{0,400}?\bdrift\b[\s\S]{0,400}?\/>/u);
  });
