...
describe("вид один — карточка (36d + zone-row.json)", () => {
  ...
  it("числа КАРТОЧКИ — 14 px с шагом 10 (пакет 55, турн 61c)", () => {
    // ПРЕЖНИЕ 6/5 БЫЛИ ЧИСЛАМИ ВВОДА. Шкала вводится огоньками-целями, между
    // которыми нужен зазор, чтобы палец не попал в соседнюю ступень; у гостя
    // она НЕ цель нажатия — «поэтому шага 10 между целями 44 нет, огоньки
    // стоят рядом». Дизайн вернул этим и второй вид (см. ниже).
    const css = read("../src/components/item/desire-scale.module.css");
    expect(css).toContain("width: 14px");
    expect(css).toContain("gap: 10px");
    // Пустой огонёк — .2; слово 12 px тоном .55. Эти два числа не менялись.
    expect(css).toContain("background: rgba(255, 249, 242, 0.2)");
    expect(css).toContain("font: 400 12px/1 var(--font-ui)");
  });

  it("ВТОРОЙ ВИД ВЕРНУЛСЯ — и вернул его дизайн, а не «а тут бы поменьше»", () => {
    // Тикет 161 снял вариант строки ПРАВИЛЬНО: у него не осталось ни одного
    // потребителя, и мёртвое правило не должно было ждать в коде. Здесь стоял
    // сторож, чтобы вариант не вернулся тихо, — и он свою работу сделал: вид
    // вернулся не тихо, а пакетом 55 (`desire-guest.json`, турн 61c) и с
    // потребителем — ПЛИТКОЙ В СЕТКЕ, где гость выбирает подарок и где шкалы
    // не было вовсе.
    //
    // Числа плитки те же, что были у снятой строки: точка 5 с шагом 4, без
    // слова («не влезает»). Совпадение не случайно — это один и тот же вопрос
    // «как показать шкалу там, где нет места словам», и дизайн ответил на него
    // так же, как отвечал раньше.
    const css = read("../src/components/item/desire-scale.module.css");
    expect(css).toContain("width: 5px");
    expect(css).toContain("gap: 4px");
    expect(css).toContain(".tile .flame");
  });

  it("вид выбирается ПРОПОМ, и по умолчанию он карточный", () => {
    const component = read("../src/components/item/desire-scale.tsx");
    expect(component).toContain('variant?: "card" | "tile"');
    // Умолчание важно: все прежние вызовы обязаны остаться карточными без
    // единой правки — иначе тихо поедет карточка вещи у гостя.
    expect(component).toContain('variant = "card"');
  });

  it("на ПЛИТКЕ пустая шкала рисуется, в карточке — нет", () => {
    // «Четыре контура — „хозяйка не сказала" тоже ответ» (61c). В сетке гость
    // сравнивает вещи взглядом, и дырка на месте шкалы читалась бы как «шкалы
    // у этой нет». В карточке вещь одна, сравнивать не с чем, и место занимает
    // слово `ItemCard.desireEmpty`.
    const component = read("../src/components/item/desire-scale.tsx");
    expect(component).toContain('if (!known && variant === "card") return null;');
  });
