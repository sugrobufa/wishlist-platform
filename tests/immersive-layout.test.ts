...
describe("все 130 зон достижимы: кадром или указателем (тикет 40)", () => {
  ...
  it("телефон: верхняя вуаль лежит НА зонах, нижняя до кадра не достаёт", () => {
    ...
    expect(touchedTop(PHONE)).toHaveLength(72);
  ...
  it("телефон: цена решения — 35 зон целиком под вуалью, и все они достижимы", () => {
    ...
    expect(swallowed).toHaveLength(36);
    ...
    expect(swallowedAt({ w: 430, h: 745 })).toBe(36);
    ...
    expect(swallowedAt({ w: 390, h: 844 })).toBe(58);
    ...
    expect(swallowedAt({ w: 360, h: 640 })).toBe(75);
...
describe("цель нажатия зоны (rooms.json → hitTargetMin)", () => {
  it("телефон: полные 44×44 у всех, кроме шести зон на самом краю окна", () => {
