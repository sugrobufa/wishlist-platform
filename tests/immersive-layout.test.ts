...
/**
 * Все 134 адреса контракта: 13 в женской комнате, 14 в мужской (money тоже —
 * ADR-0003; полка «Бар и табак» — решение владельца 14.08.2026, тикет 234).
 */
...
const BEYOND_PHONE_WINDOW = [
  ...
  "warm/home",
  "warm/books",
  // Тикет 234: проигрыватель на мраморной консоли (447…538) — в покое телефона
  // до него не дотянуться, как и до всей правой трети кадра.
  "warm/music",
  "warm/flowers",
  "lux/books",
  "lux/flowers",
  ...
  "gamer/books",
  "gamer/money",
  // Тикет 234: стеклянная шкатулка с часами дошла до самого края кадра (630).
  "sport/watches",
  "sport/books",
  "sport/music",
  "sport/sport",
...
const DESKTOP_REACH: Array<{
  ...
  { screen: { w: 1024, h: 768 }, bleedX: 61.44, field: 64, full: 131, short: 3, gone: 0 },
  { screen: { w: 1194, h: 834 }, bleedX: 71.64, field: 43.875, full: 132, short: 2, gone: 0 },
  { screen: { w: 1280, h: 720 }, bleedX: 5.12, field: 0, full: 134, short: 0, gone: 0 },
  { screen: DESKTOP, bleedX: 76.8, field: 0, full: 132, short: 2, gone: 0 },
  { screen: { w: 1280, h: 1024 }, bleedX: 76.8, field: 112, full: 132, short: 2, gone: 0 },
  { screen: { w: 1366, h: 768 }, bleedX: 5.128, field: 0, full: 134, short: 0, gone: 0 },
  { screen: { w: 1440, h: 900 }, bleedX: 86.4, field: 0, full: 133, short: 1, gone: 0 },
  { screen: { w: 1536, h: 864 }, bleedX: 6.144, field: 0, full: 134, short: 0, gone: 0 },
  { screen: { w: 1920, h: 1080 }, bleedX: 7.68, field: 0, full: 134, short: 0, gone: 0 },
];
 ...
 */
/**
 * ТИКЕТ 238 УБРАЛ ОТСЮДА ТРИ КОПИЛКИ. `gamer/money`, `sport/money` и
 * `loft/money` недобирали до 44 px не потому, что стоят у правой стены, а
 * потому что их прямоугольники были 18×20, 18×25 и 20×23 — точки РЯДОМ с
 * банкой, а не сами банки. Переразметка по предмету дала им 34×66, 40×69 и
 * 36×59, и правый край окна их больше не подъедает. Осталось двое, и у обоих
 * причина настоящая: узкие предметы у самой правой стены.
 */
const DESKTOP_1280_SHORT = ["lux/home", "study/watches"];
...
describe("комната во весь экран: кроп вместо полей (тикет 42)", () => {
  ...
  it("CSS телефона считает ту же формулу и кладёт вуаль НА комнату (тикет 57)", () => {
    ...
    // Потолок листа — прежнее правило («не выше нижней четверти кадра»), а
    // низ с тикета 140 отсчитывается от `--sheet-bottom`: на телефоне под
    // листом стоит таб-бар (86), а не полоса указателя (116), и разница в
    // 30 px была той самой щелью, сквозь которую владелец видел чужой список.
...
// ---------------------------------------------------------------------------
describe("все 134 зоны достижимы: кадром или указателем (тикет 40)", () => {
  it("134 адреса в контракте — по 13 в женской комнате, по 14 в мужской", () => {
    expect(allZones).toHaveLength(134);
    expect(roomsContract.rooms).toHaveLength(10);
  });

  it("телефон, окно В ПОКОЕ: 99 зон пальцем, 35 за краем — и это все 134", () => {
    ...
    // 33 стало 35 (тикет 234): к правой трети кадра прибавились `warm/music` и
    // `sport/watches` — оба предмета, найденных пакетом 50, стоят у правой
    // стены комнаты. Это не ухудшение, а та же геометрия: дорога к ним — пан и
    // указатель, ровно как у прежних 33.
    expect(beyond).toHaveLength(35);
    expect(reachableByFinger).toHaveLength(99);
  ...
  it("десктоп: на 1280×800 пальцем достаются 132 зоны, 2 срезаны краем окна", () => {
  ...
  it("десктоп: разбиение по вьюпортам — сумма всегда 134", () => {
  ...
  it("десктоп 1024×768 — порог раскладки, и он повторяет эталон зона в зону", () => {
    ...
    //
    // ШЕСТАЯ ЗОНА ПОЯВИЛАСЬ РОВНО НА САМОМ УЗКОМ ОКНЕ (тикет 234) — и она
    // первая, кого подъедает ЛЕВЫЙ край, а не правая стена комнаты. Плёночная
    // камера `study/tech` стоит на сундуке у самого края кадра (21…57), и на
    // 1024×768 кроп оставляет ей 40 px вместо 44. Цель не срезана, зону видно,
    // вторая дорога — указатель. Разница названа списком, а не смягчённым
    // сравнением: приедет седьмая — тест назовёт её здесь.
    const NARROW_EXTRA: Record<string, string[]> = { "1024×768": ["study/tech"] };
    for (const screen of DESKTOP_NARROW) {
      const gone = allZones.filter(({ rect }) => !tappable(rect, "desktop", screen));
      const short = allZones.filter(
      ...
      expect(
        short.map((z) => z.id).sort(),
        where,
      ).toEqual([...DESKTOP_1280_SHORT, ...(NARROW_EXTRA[where] ?? [])].sort());
    }
  ...
  it("ДОРОГА 2 на десктопе: всё, что срезал кроп, лежит в указателе", () => {
    ...
    expect(listedInIndex.size).toBe(126);
  ...
  it("телефон: верхняя вуаль лежит НА зонах, нижняя до кадра не достаёт", () => {
    ...
    // 74, а не 75: тикет 81-2 опустил `gamer/sneakers` с пустого проёма ниши на
    // саму обувь (y 182 → 203), и на эталонном телефоне зона вышла из-под вуали.
    // 72, а не 74: тикет 233 опустил `gamer/sport` с голой стены на скамью с
    // гантелью (y 164 → 219) и `sport/grooming` со стены у зеркала на поднос с
    // уходом (y 170 → 206) — обе вышли из-под вуали.
    // 73, а не 72: тикет 234 поднял `study/tech` — плёночная камера стоит на
    // крышке сундука (y 228), и на эталонном телефоне её верх задевает вуалью.
    // 74, а не 73: тикет 238 сдвинул трёх, и они посчитались в разные стороны.
    // Вошли под вуаль двое — `gamer/books` (со шкатулки часов на полку с
    // корешками, y 215 → 127) и `loft/watches` (с кромки полки на шкатулку
    // выше, y 207 → 156). Вышел из-под неё один — `sport/tech`: ноутбук стоит
    // на столе ниже зеркала (y 137 → 191). Итог +2 −1.
    expect(touchedTop(PHONE)).toHaveLength(74);
    // 92, а не 91: тикет 75 поднял `loft/sneakers` с пола на саму обувь
    // (y 222 → 196), и на этом экране зона въехала под вуаль.
    // 93, а не 92: тикет 81-2 поднял `lux/travel` с чемоданов на раскрытый кейс
    // (y 242 → 208) — запас под откинутую крышку, и зона въехала под вуаль.
    // 92, а не 93: тикет 233 опустил `gamer/sport` на скамью; `sport/grooming`
    // на этом экране под вуалью и осталась — вуаль та же, а кадр ниже.
    ...
    // 100, а не 98: тикет 81-2 поднял `lux/travel` (y 242 → 208) и `lux/jewelry`
    // (y 218 → 214, с голой столешницы на её кусок рядом с парфюмом) — на этом
    // экране под вуаль въехали обе.
    // 99, а не 100: тикет 233, `gamer/sport` — та же причина.
    // 98, а не 99: на самом низком телефоне `sport/tech` ушёл из-под вуали
    // вместе со всеми (низ 165 → 229), а вошедшие `gamer/books` и `loft/watches`
    // здесь были под ней и раньше — кадр ниже, вуаль та же.
    ...
    // вуали вышла.
    // 103, а не 102: тикет 81-2 поднял `lux/travel` (y 242 → 208) — на самом
    // коротком экране зона въехала под вуаль.
  ...
  it("телефон: цена решения — 34 зоны целиком под вуалью, и все они достижимы", () => {
    ...
    // 36, а не 35: тикет 233 переставил `loft/events` с чёрной колонны на веер
    // билетов с их доской (y 165 → 82) — веер висит высоко, и зона утонула под
    // вуаль целиком. Дороги к ней обе на месте, они проверяются ниже.
    // 34, а не 36: тикет 234 ВЫВЕЛ из-под вуали двоих. `sport/watches` уехала
    // со служебного места посреди кадра на витрину с часами (y 150 → 172), а
    // `study/tech` — с края стола на сундук у пола (y 144 → 228). Обе зоны
    // тонули под вуалью ровно потому, что стояли не на предмете.
    // 33, а не 34: тикет 238 вывел из-под вуали двоих и утопил одного.
    // Вышли `emerald/music` (с полки над вертушкой на саму вертушку с мраморной
    // консолью, низ 180 → 222) и `sport/tech` (со стены у зеркала на ноутбук,
    // низ 165 → 229). Утонул `gamer/books`: полка с корешками висит высоко
    // (низ 280 → 166). Обе дороги к утонувшей на месте, они проверяются ниже.
    expect(swallowed).toHaveLength(33);
    ...
    // 33, а не 34 — по той же причине, что на эталоне: −`emerald/music`,
    // −`sport/tech`, +`gamer/books`.
    expect(swallowedAt({ w: 430, h: 745 })).toBe(33);
    // 56, а не 55: тикет 75 подрезал `loft/fashion` по рейлу с одеждой
    // (низ 214 → 191), и на этом экране зона утонула под вуаль целиком.
    // 58, а не 56: тикет 81-2 поднял низ `lux/money` (211 → 208, конверт вместо
    // конверта с отражением) и ужал `lux/music` (низ 216 → 208, objectAbsent
    // уступает полосу крышке кейса) — обе на этом экране ушли под вуаль целиком.
    // На 390×844 и 375×667 счёт не сдвинулся: тикет 233 утопил `loft/events` и
    // ровно столько же вывел из-под вуали (`sport/grooming` встала на поднос).
    // Те же двое вышли из-под вуали и на узких экранах: 58 → 56, 66 → 64.
    expect(swallowedAt({ w: 390, h: 844 })).toBe(56);
    expect(swallowedAt({ w: 375, h: 667 })).toBe(64);
    // 75, а не 77: на самом коротком экране `loft/events` тонула и до переезда,
    // а `gamer/sport` и `sport/grooming` из-под вуали вышли.
    // 72, а не 73: на самом узком экране кадр ниже всего, и вуаль накрывает
    // больше половины зон — поэтому из трёх сдвинутых тикетом 238 счёт изменил
    // ровно один. Который именно, здесь не называем: проверка считает ЧИСЛО, а
    // имена держит проверка эталонного телефона выше.
    expect(swallowedAt({ w: 360, h: 640 })).toBe(72);
  ...
  it("десктоп: полосы лежат НА зонах, и это норма — нажатий они не берут", () => {
    ...
    // 32 зоны, на 1280×800 — 40, на 1024×768 — 28 (тикет 53 переставил
    ...
    //
    // Тикет 63 УМЕНЬШИЛ три из четырёх чисел (34→32, 41→40, 30→28), и это не
    // послабление: `study/travel` и `loft/travel` переехали с фронта нижнего
    // сундука на верхний, то есть ВВЕРХ, и их нижние кромки (338 и 336 в кадре)
    // поднялись до 312 и 290 — из-под нижней полосы вышли. На 1280×800 из двоих
    // успел выйти только `loft/travel`: у `study/travel` низ 312 всё ещё под
    // полосой. На 1280×1024 их не было и раньше — там кадр ниже полос.
    ...
    // Тикет 234 прибавил три РОВНО НА ЭТАЛОНЕ (40 → 43), и ни одной на
    // остальных окнах. Причина у обеих половин одна — высота кадра: на
    // 1280×800 полосы лежат на комнате, и под них попали все три новые зоны
    // (`warm/music` и `sport/watches` поднялись на предметы у правой стены,
    // камера `study/tech` встала на сундук у пола), а на 1920×1080, 1024×768
    // и 1280×1024 кадр от полос отстоит, и до новых зон они не достают.
    // ТИКЕТ 238 ПРИБАВИЛ ПО ДВЕ НА ОБОИХ ШИРОКИХ ОКНАХ (32 → 34, 43 → 45), и
    // причина одна: три копилки выросли ВНИЗ. Прежние прямоугольники были
    // точками рядом с банкой (18×20, 18×25, 20×23) и до нижней полосы не
    // доставали; новые берут банку со столешницей и кончаются на 330, 339 и 337
    // — под полосой. Вышел из-под верхней полосы один `gamer/music`: аудиогруппа
    // стоит ниже, чем стояла пустая стена над ней (верх 28 → 77). Итог +3 −1.
    // 35, а не 34: `cottage/flowers` взяла кувшин до дна (низ 282 → 347) и
    // достала нижнюю полосу.
    expect(touched({ w: 1920, h: 1080 })).toHaveLength(35);
    expect(touched(DESKTOP)).toHaveLength(46);
    ...
    // 30, а не 27: три копилки выросли вниз до 330…339 и достали нижнюю полосу
    // на этом окне тоже, к ним прибавился кувшин `cottage/flowers` (низ 347);
    // `gamer/music` из-под верхней вышел. +4 −1.
  ...
  it("а на десктопе — зависит, и это разница между вписыванием и кропом", () => {
    ...
    expect(shortAt(DESKTOP)).toHaveLength(2);
    // 1, а не 3: тикет 238 дал копилкам «Геймера» и «Спорта» настоящие
    // прямоугольники банок, и до 44 px они теперь добирают. Осталась одна
    // `lux/home` — предмет у самой правой стены, к кропу это отношения не имеет.
    expect(shortAt({ w: 1440, h: 900 })).toHaveLength(1);
    expect(shortAt({ w: 1440, h: 900 })).toEqual(["lux/home"]);
    ...
    // Общий список у окон у́же эталона стал общим «плюс одна»: на самом узком
    // 1024×768 левый край подъедает ещё и `study/tech` (тикет 234, разбор — в
    // проверке порога раскладки выше).
    expect(shortAt({ w: 1024, h: 768 }).sort()).toEqual(
      [...DESKTOP_1280_SHORT, "study/tech"].sort(),
    );
  ...
  it("«зона целиком в окне» и «есть цель нажатия» — разные вещи, и обе посчитаны", () => {
    // 70 зон попадают в окно ЦЕЛИКОМ, ещё 27 — краем: 15 свешиваются влево
    ...
    //
    // Было 68 и 29 при 17 свешенных. Тикет 63 перевёл две зоны из «краем» в
    ...
    // Тикет 234: 74 → 75 целиком (прибавилась `study/tech` — камера стоит на
    // 21…57, внутри окна 12…442), «краем» 23 → 24 (`study/bar`: барная тележка
    // упирается в правую кромку окна, 425…483). Сумма выросла до 134 вместе с
    // картой: 75 + 24 + 35.
    expect(whole).toHaveLength(75);
    ...
    expect(whole.length + partial.length + BEYOND_PHONE_WINDOW.length).toBe(134);
...
describe("цель нажатия зоны (rooms.json → hitTargetMin)", () => {
  it("телефон: полные 44×44 у всех, кроме семи зон на самом краю окна", () => {
    ...
    // Было семь. `sport/events` из списка ВЫШЛА: тикет 233 взял рамку с
    // билетами целиком, зона стала шире и левее (415…453 → 385…457), и окно в
    // покое показывает 57 px вместо 27 — больше порога. Это побочный, но
    // закономерный итог правила «брать предмет вместе с рамой»: рама шире
    // предмета, и цель нажатия растёт вместе с ней.
    // Стало семь: тикет 234 добавил `study/bar`. Барная тележка стоит на
    // 425…483, а окно в покое кончается на 442 — пальцу достаётся 17 px из 58.
    // Тот же долг, что у `bold/music`: дорога к ней — пан и указатель.
...
describe("166 — шапка и знак угла начинаются с одной линии (телефон)", () => {
  // Приёмка владельца 10.08: «надпись „Комната такого-то" и правый символ
  // сокровищницы не выровнен, комнату надо приподнять».
  //
  // ЧИСЛА, ПОЧЕМУ ОН ПРАВ. Знак угла стоял на 12 от верха, блок заголовка —
  // на 44 (`--imm-title-top`): замер на 375 давал плашку 12…56 и блок 12…93.5.
  // Два числа пришли из разных решений пакета и никогда не сводились вместе.
  //
  // ТОЛЬКО ТЕЛЕФОН. На десктопе пара УЖЕ выровнена, и по ЦЕНТРАМ: знак стоит
  // четвёртой колонкой сетки шапки с `align-items: center` (замер 1280: блок
  // 44…101.5, ряд знаков 50.8…94.8 — центры 72.75 и 72.8). Телефон ломался
  // потому, что правой колонки у него нет и знак выпадает из сетки в
  // `position: absolute`.
  const globals = readFileSync(
    fileURLToPath(new URL("../src/app/globals.css", import.meta.url)),
    "utf8",
  ...
  const phoneBranch = (() => {
    const at = globals.indexOf("@media not all and (min-width: 1024px)");
    return at === -1 ? "" : globals.slice(at, globals.indexOf("\n}\n", at));
  })();

  it("отступ угла — ОДНА переменная на двоих, а не два числа в двух правилах", () => {
    // Пока числа стояли порознь, сводились они только на глаз — и разъехались.
    expect(globals).toMatch(/\.imm \{[\s\S]*?--imm-corner-top: 12px;/u);
    expect(globals).toMatch(
      /\.imm-corner \{[\s\S]*?top: calc\(var\(--imm-corner-top\) \+ var\(--imm-safe-top, 0px\)\);/u,
    );
  });

  it("на телефоне шапка равняется по знаку, на десктопе — по числу пакета", () => {
    expect(
      phoneBranch,
      "шапка телефона перестала равняться по знаку угла — заголовок снова уедет вниз",
    ).toMatch(
      /\.imm-rail-top \{\s*padding-top: calc\(var\(--imm-corner-top\) \+ var\(--imm-safe-top, 0px\)\);/u,
    );
    // Базовое правило (оно же десктопное) держит число пакета: 44 остаётся
    // контрактом там, где выравнивание делает сетка.
    expect(globals).toMatch(
      /\.imm-rail-top \{[\s\S]*?padding-top: calc\(var\(--imm-title-top\) \+ var\(--imm-safe-top, 0px\)\);/u,
    );
    expect(immersiveLayout.phone.titleTop, "число пакета не трогаем").toBe(44);
  });

  it("двенадцать не переписано числом в телефонной ветке", () => {
    // Литерал в ветке — это и есть второе число, с которого всё началось.
    expect(phoneBranch).not.toMatch(/padding-top: calc\(12px/u);
  });
});
