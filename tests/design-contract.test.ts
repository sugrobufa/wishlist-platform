...
describe("design handoff contract", () => {
  ...
  it("ДОЛГ ДИЗАЙНУ ЗАКРЫТ: отставших bloomAR не осталось ни одного", () => {
    ...
    // Тикет 238 переразметил десять зон, стоявших на пустом месте, но в реестр
    // добавил ДВЕ: `sport/tech` и `loft/grooming` переразмечены впервые, у
    // остальных восьми `rectOld` уже был с раундов 4–5 и просто обновился.
    // 71 стало 73. Пятно пересчитано по формуле всем десяти — проверка `stale`
    // выше осталась нулём, и это её первая работа после заведения.
    expect(allZones.filter(({ zone }) => zone.rectOld)).toHaveLength(73);
  });
...
describe("кадры «открыто» (openFrame — единственный источник истины)", () => {
  ...
  it("КОПИЯ КОНТРАКТА ИЗ ПАКЕТА 40 НЕ ПРИНЯТА ЦЕЛИКОМ — и вот чем она отличается", () => {
    ...
    // расхождений не было ни одного. Теперь их ДВАДЦАТЬ ОДНО, и все наши:
    // пакет 50 прислал не координаты, а ПРАВИЛО («прямоугольник берёт предмет
    // вместе с тем, что его держит»), измеряет прямоугольники по уставу
    // разработка, и все двадцать измерены нами по базовым кадрам. Десять из них
    // добавил тикет 238 — обход, показавший, что промахов было больше, чем
    // чинил тикет 233, и одиннадцатое (`cottage/flowers`) — единственный из
    // четырёх пограничных случаев, где правило опоры ничего не стоит.
    // Список закрытый: двадцать второе расхождение обязано быть объяснено
    // здесь же, а не появиться молча.
    //
    ...
    expect({
      ...
      accepted: 134,
      // 50 + три зоны тикета 238, у которых причины пересъёмки не было вовсе
      // (`sport/tech`, `loft/grooming` переразмечены впервые, у остальных семи
      // мужских адресов строка уже стояла с прежних переразметок).
      reshootReason: 54,
      // 16 + 7 разборов переразметки тикета 233 + 4 тикета 234 (три предмета,
      // найденные пакетом 50, и новая полка `study/bar`) + 10 тикета 238.
      remapNote: 38,
      eyeNote: 12,
      ...
      // + три предмета тикета 234 + `sport/tech` и `loft/grooming` тикета 238
      // (остальные восемь его правок «было» уже имели).
      rectOld: 73,
...
describe("пересечения прямоугольников зон", () => {
   ...
   *
   * ТИКЕТ 238 ЗАКРЫЛ ДВЕ, ЗАВЁЛ ОДНУ И РАСТИЛ ДВЕ. Десять прямоугольников
   * стояли на пустом месте и переехали на свои предметы; пары поехали следом.
   *   • `gamer/watches×books` 344 px² — ЗАКРЫТА. Пара была ЧИСТЫМ СЛЕДСТВИЕМ
   *     промаха: `books` стоял на шкатулке с часами, то есть на предмете
   *     соседней зоны. Обе уехали на свои полки, и общего у них не осталось.
   *   • `loft/sport×watches` 165 px² — ЗАКРЫТА тем же способом: `watches` сидел
   *     на кромке полки с винилом внутри прямоугольника «Спорта», а часы стояли
   *     полкой выше.
   *   • `sport/grooming×tech` 161 px² — НОВАЯ, и она честная. Ноутбук и поднос
   *     с флаконами стоят на ОДНОМ столе вплотную: левый край ноутбука 264,
   *     правый край подноса 271. Подрезать ноутбук ради нуля значило бы срезать
   *     угол экрана — по правилу пакета 50 предмет режут последним. 7×23.
   *   • `emerald/books×music` 2496 → 2970. Вертушка СТОИТ ВНУТРИ книжного
   *     шкафа, и `books` обводит шкаф целиком: вложение по построению, тот же
   *     случай, что `study/tech×travel`. Прежнее число было меньше не потому,
   *     что предметы расходились, а потому что прямоугольник `music` стоял на
   *     пустой полке НАД вертушкой и захватывал шкаф меньшей площадью.
   *   • `sport/sport×money` 450 → 1800. Банка-копилка стоит на столе ПЕРЕД
   *     велосипедом — по кадру она его закрывает, и развести их нельзя ничем,
   *     кроме перерисовки кадра. Прежние 450 — площадь пересечения велосипеда
   *     с прямоугольником, в котором копилки не было вовсе.
   */
  const OVERLAP_DEBT = [
  ...
  it("РЕЕСТР И ЭКРАН СНОВА СОВПАДАЮТ СТРОКА В СТРОКУ", () => {
    ...
    // 15 → 14: тикет 238 закрыл две пары (`gamer/watches×books`,
    // `loft/sport×watches`) и завёл одну (`sport/grooming×tech`).
...
describe("флаги проверки: notClamped / eyeChecked / wrongTarget", () => {
  ...
  it("notClamped — машинное правило прежней системы, снятое с переразмеченных", () => {
    ...
    // Тикет 238 забрал в раунд 50 ещё девять: три из счёта раунда 4
    // (`emerald/music`, `gamer/music`, `cottage/flowers`) и шесть из счёта раунда 5
    // (`gamer/watches`, `gamer/books`, `gamer/money`, `sport/money`,
    // `loft/watches`, `loft/money`). 32 → 29 и 10 → 4. Две оставшиеся правки
    // тикета — `sport/tech` и `loft/grooming` — переразмечены ВПЕРВЫЕ и ни из
    // какого счёта не уходили: до них `remappedRound` у зон не было вовсе.
    expect(remapped.filter(({ zone }) => zone.remappedRound === 4)).toHaveLength(29);
    expect(remapped.filter(({ zone }) => zone.remappedRound === 5)).toHaveLength(4);
    ...
    // Тикет 238 дописал в тот же раунд ДЕСЯТЬ — те, что нашёл обход тикета 233
    // и назвать которые внутри чужого тикета было нельзя. Это НЕ новое правило:
    // то же правило пакета 50, применённое к прямоугольникам, у которых предмета
    // не было ни одного. Три копилки (`gamer/money`, `sport/money`,
    // `loft/money`) промахнулись одинаково — точка ставилась РЯДОМ с банкой; две
    // пары в стеллажах стояли на предмете соседней зоны (`gamer/books` на
    // шкатулке часов, `gamer/watches` на дверцах тумбы; `loft/watches` на кромке
    // полки с винилом); две «музыки» лежали на пустой полке над вертушкой
    // (`emerald/music`) и на голой стене над аудиогруппой (`gamer/music`);
    // `sport/tech` стоял на стене левее зеркала, `loft/grooming` — на полу под
    // верстаком.
  ...
  it("от осмотра раунда 5 осталось четверо — шестерых домерил тикет 238", () => {
    ...
    //
    // ТИКЕТ 238 ЗАБРАЛ ШЕСТЬ ИЗ ОСТАВШИХСЯ ДЕСЯТИ, и это приговор осмотру
    // раунда 5, а не его продолжение. `gamer/watches`, `gamer/books`,
    // `gamer/money`, `sport/money`, `loft/watches`, `loft/money` — ровно те
    // «Книги, Часы и Деньги мужских комнат», про которые раунд 5 сказал, что
    // подвинул их НА предметы. Обход тикета 233 показал глазами, что ни один из
    // шести на предмете не стоял: два сидели на предмете СОСЕДНЕЙ зоны, три
    // копилки — рядом с банкой, одни часы — на кромке полки с винилом.
    // Правым краем за 430 они при этом честно уехали, поэтому число ниже
    // ничего и не поймало: оно проверяло СТОРОНУ кадра, а не предмет.
    ...
    // Все четверо переживших стоят правее 430 — и это ровно то, почему число
    // ниже шестерых промахнувшихся не поймало: правый край у них был честный.
    const toRightThird = fixed.filter(({ zone }) => zone.rect.x + zone.rect.w > 430);
    expect(toRightThird).toHaveLength(4);
  });
