...
describe("в одной зоне не хотят того, что там уже своё", () => {
  ...
  it("числа стенда: cream 54/26/80 (13 зон), warm 54/26/80 (13 зон)", () => {
    ...
    //
    // «ДВЕНАДЦАТИЗОННЫХ» ПРЕСЕТОВ БОЛЬШЕ НЕТ (тикет 234). У `warm` не хватало
    // ровно «Музыки»: предмета в интерьере не находили, и адрес был скрыт
    // продуктом с ответом 404. Пакет 50 нашёл проигрыватель на мраморной
    // консоли, прямоугольник измерен — полка вернулась, и пресет стал полным.
    // Вместе с полкой вернулись и её вещи: посев идёт по пресету, а «Музыки» в
    // пресете не было вовсе. Отсюда 49/24/73 → 54/26/80 — ровно набор пула
    // `music`, и числа «Тихой роскоши» сравнялись с «Кремовой».
    expect(presetZones("warm"), "warm — теперь полный, 13 полок").toHaveLength(13);

    expect(countsFor("cream")).toEqual({ room: 54, hall: 26, all: 80 });
    expect(countsFor("warm")).toEqual({ room: 54, hall: 26, all: 80 });
  });
