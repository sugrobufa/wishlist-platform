...
describe("takenForRoomSlug — хозяйке своей комнаты канал отвечает пустотой", () => {
  it("вошла хозяйка: itemIds/mine пусты, myBookingsCount 0 — как будто не занято ничего", async () => {
    ...
    expect(forOwner).toEqual({
      ...
      myBookingsCount: 0,
      signedIn: true,
      // Тикет 250: канал говорит «ты хозяин ЭТОЙ комнаты», и на этом всё.
      // Про брони флаг не говорит ничего — списки как были пустыми, так и
      // остались; ради него экран и перестал требовать заполнить форму, чтобы
      // узнать про запрет. Сверка идёт целым объектом нарочно: добавится сюда
