...
describe("телефон не тронут ни одним числом", () => {
  it("базовые правила блока — те же, что были до 209", () => {
    ...
    // друг под другом, потому что иначе не помещаются». Числа — макет 41a и
    // замер тикета 199.
    //
    // ПРОСВЕТЫ БЛОКА ЗДЕСЬ НЕ ПРОВЕРЯЮТСЯ НАРОЧНО, тем же порядком, что и метка
    // пустой строки ниже: они принадлежат тикету 255 (`fit.breakdown` пакета 44
    // называет три разных числа вместо одного общего `gap`) и стерегутся его
    // тестом — tests/empty-room-first-screen.test.ts, числами из самого
    // контракта. Здесь остаётся только то, за что отвечает 209: общего `gap` у
    // блока нет и вернуться он не должен — с ним три просвета снова станут одним.
    expect(/\.imm-empty-start \{([^}]*)\}/u.exec(css)?.[1] ?? "").not.toMatch(/gap:/u);
