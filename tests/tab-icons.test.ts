 ...
 *
 * ЧТО ПРИВЁЗ РАУНД 42 (тикет 178) — ОТВЕТ НА ПИСЬМО 45, дельтой, а не файлом
 * целиком. Две правки, обе точечные: поле `rows` у `tab-room.svg` стало
 * `["бар"]`, ряд «угол» — тремя знаками. Довод дизайна — наш же: в углу комнаты
 * знака комнаты быть не может, человек уже в ней. И вместе с правкой приехало
 * правило сторожа, которого у нас не было: **знак, у которого поле `rows`
 * называет ряд, где его на экране нет** (`rowComposition` ниже).
 */
 ...
 * каждого знака. В раунде 40 списки расходились в семи знаках, и расхождение
 * было не в пользу одной из сторон: блок не знал четырёх рядов вовсе
 * («аватар», «лист действий», «строка списка зоны», «профиль»), а поля знаков
 * не знали, что плюс стоит ещё и в баре гостя. Возьми мы одну сторону —
 * вторая половина пар не проверялась бы молча, а молчание тут и есть та
 * болезнь, которую лечит весь тикет. Берём объединение: пропуск любой из
 * сторон закрыт другой.
 *
 * РАСХОЖДЕНИЯ СЕГОДНЯ НЕТ (раунд 41 пересобрал блок из полей; пустоту сторожит
 * тест «две мелочи письма 44 закрыты»), а объединение осталось — и раунд 42
 * показал, зачем. Оно не костыль под вчерашнее расхождение: `rowComposition`
 * ищет фантом в ОБОИХ списках, потому что назови ряд одно лишь поле знака —
 * фантом обязан найтись всё равно. Какой из списков полнее, сторожу знать не
 * нужно ни сегодня, ни завтра.
 */
const rowsToGuard = (c: IconsContract = contract): Map<string, string[]> => {
  const rows = new Map<string, Set<string>>();
  const add = (row: string, files: string[]) =>
    rows.set(row, new Set([...(rows.get(row) ?? []), ...files]));
  for (const [row, files] of Object.entries(c.rows)) add(row, files);
  for (const [file, entry] of Object.entries(c.set)) for (const row of entry.rows) add(row, [file]);
...
/**
 * СТОРОЖ СОСТАВА РЯДА — В ОБЕ СТОРОНЫ (тикет 178, письмо 45, раунд 42).
 *
 * ЗАЧЕМ ЕЩЁ ОДИН. `rowViolations` выше сторожит СИЛУЭТЫ: два знака в одном ряду
 * читаются одинаково. Этот — про СОСТАВ, и болезнь у него своя, с двумя
 * сторонами:
 *
 *   `extra`   — код рисует знак, которого контракт в ряду не называет. Наша
 *               самодеятельность, красная всегда.
 *   `phantom` — КОНТРАКТ НАЗЫВАЕТ ЗНАК, КОТОРОГО НА ЭКРАНЕ НЕТ. Вот эта
 *               сторона до раунда 42 не сторожилась вовсе: старая проверка
 *               умела только «присутствие», а отсутствие прикрывала списком
 *               ожидания.
 *
 * ПОЧЕМУ ОБРАТНАЯ СТОРОНА ВАЖНЕЕ, ЧЕМ КАЖЕТСЯ. Ровно ею болел раунд 41: блок
 * `rows` пересобрали ИЗ ПОЛЕЙ знаков, поле у `tab-room.svg` говорило «бар,
 * угол» — и знак комнаты приехал в угол комнаты, туда, где человек уже стоит.
 * Пересборка тиха: ни один сторож присутствия на неё не отзывается, потому что
 * лишний знак ничему не мешает — его просто некому нарисовать. Дизайн завёл у
 * себя ту же проверку и написал прямо: «первым он поймал бы tab-room в углу».
 *
 * ЧИТАЕТ ОБА СПИСКА КОНТРАКТА, а не блок `rows` (`rowsToGuard`). Это и есть
 * второе отличие от вчерашней проверки: назови ряд ТОЛЬКО поле знака, а блок
 * промолчи — фантом всё равно найдётся. Один список полнее другого сегодня,
 * завтра наоборот, и зависеть от этого сторожу нельзя.
 *
 * СРАВНИВАЕТСЯ ПО СМЫСЛУ, А НЕ ПО ИМЕНИ ФАЙЛА: у витрины три файла одной
 * геометрии (`tab-`/`action-`/`ui-`), и опознание компонента вернуло бы любой
 * из трёх — сверка имён падала бы на порядке чтения папки.
 */
function rowComposition(
  row: string,
  drawn: string[],
  c: IconsContract = contract,
): { extra: string[]; phantom: string[] } {
  const named = [...new Set((rowsToGuard(c).get(row) ?? []).map(meaningOf))];
  return {
    extra: [...new Set(drawn.filter((m) => !named.includes(m)))].sort(),
    phantom: named.filter((m) => !drawn.includes(m)).sort(),
  };
}
 ...
 *
 * ЧИНИТЬ БЫЛО НЕ НАШЕ ДЕЛО. Поставить четвёртый знак значило бы выполнить чужой
 * файл против смысла экрана; вычеркнуть строку из контракта — сочинить контракт
 * за дизайн. Правило то же, что у мелочей письма 44: **чужие данные не чиним и
 * не сочиняем, фиксируем числом, чтобы изменение позвало.** Выписано письмом 45.
 *
 * ПОЗВАЛО (тикет 178, раунд 42). Дизайн ответил дословно нашим доводом — «вы
 * правы, и довод сильнее любых чисел» — и убрал знак из обоих своих списков:
 * поле `rows` у `tab-room.svg` стало `["бар"]`, ряд «угол» — тремя знаками.
 * Строчку убрал не человек, а красный: пока она стояла здесь, проверка
 * равенства требовала фантом НАЙТИ, а по новым данным его нет. Ровно так список
 * и обязан пустеть.
 *
 * ПРОВЕРКА НА РАВЕНСТВО, как и у `PENDING`: список не растёт молча (назовёт
 * дизайн в ряду знак, которого экран не рисует, — красный) и не залёживается.
 * Обратная сторона строже некуда: знак, который рисует КОД и которого нет в
 * контракте, не записывается сюда никогда — это уже наша самодеятельность, и
 * она красная сразу.
 *
 * И ГЛАВНОЕ — СПИСОК БОЛЬШЕ НЕ ЕДИНСТВЕННЫЙ СТОРОЖ ЭТОГО МЕСТА. Вчера пустой
 * список означал бы «никто не смотрит»: сама проверка читала только блок `rows`
 * и молчала, назови ряд одно лишь поле знака. Теперь под ним лежит
 * `rowComposition` — обе стороны, оба списка контракта, и проверен он на
 * подложке ниже.
 */
...
describe("сторож ряда — ПО СИЛУЭТУ, а не по геометрии (тикеты 151, 165)", () => {
  ...
  /**
   * СМЫСЛ КОМПОНЕНТА — ПО ГЕОМЕТРИИ, а не по имени переменной. Тем же способом,
   * каким компоненты сверяются с набором наверху: имён знаков в коде сторожа
   * нет ни одного, и подмена рисунка меняет ответ сама.
   */
  ...
  /**
   * РЯДЫ, КОТОРЫЕ МЫ УМЕЕМ СНЯТЬ С ЭКРАНА, — и это честная граница сторожа
   * состава. Рядов в контракте шестнадцать; разметкой у нас взяты два — бар
   * (`tab-slots.tsx`) и угол сцены (`app/room/page.tsx`, знаки тикета 135).
   * Остальные четырнадцать сторожатся по силуэтам, но не по составу: снимать их
   * с экрана пока нечем, а «своё представление о разметке» — не экран.
   */
  const SCREEN_ROWS = [
    ["бар", barRow],
    ["угол", cornerRow],
  ...
  /** Состав ряда на ЭКРАНЕ — смыслами, снятыми с разметки. */
  const drawnMeanings = (where: string, names: readonly string[]): string[] =>
    names.map((name) => {
  ...
  it("список «ждём ответа по составу ряда» — те же три требования (письмо 45)", () => {
    ...
    for (const item of ROW_PENDING) {
      ...
      //    прикрывает пустое место (а завтра прикроет непустое). Смотрим в оба
      //    списка контракта, как и сам сторож: назови ряд одно лишь поле знака
      //    — строчка всё равно законна.
      const row = guardedRows.get(item.row);
  ...
  it("СОСТАВ РЯДА РАВЕН ЭКРАНУ В ОБЕ СТОРОНЫ (тикет 178, письмо 45)", () => {
    ...
    //
    // ЧТО ДОБАВИЛ РАУНД 42. Обратную сторону и второй список. Раньше здесь
    // читался только блок `rows`, а фантом («контракт зовёт знак, которого на
    // экране нет») прикрывался списком ожидания — то есть сторожилось
    // ПРИСУТСТВИЕ, а отсутствие ждало письма. Теперь обе стороны считает
    // `rowComposition`, и читает он ОБА списка контракта: блок и поля `rows` у
    // знаков. Дизайн завёл у себя ту же проверку тем же раундом.
    //
    ...
    //
    // СТОРОНЫ НЕ РАВНОПРАВНЫ (тикет 169). Знак, который рисует КОД мимо
    // контракта, — наша самодеятельность, и она красная всегда. Знак, который
    // называет КОНТРАКТ, а код не рисует, бывает и нашей забывчивостью, и чужой
    // ошибкой; чужие — выписаны в `ROW_PENDING` с номером письма и сверяются на
    // РАВЕНСТВО. Сегодня список пуст, и фантомов быть не должно ни одного.
    for (const [row, drawn] of SCREEN_ROWS) {
      const { extra, phantom } = rowComposition(row, drawnMeanings(row, drawn));
      expect(extra, `ряд «${row}»: код рисует знак, которого в контракте нет`).toEqual([]);
      expect(
        phantom,
        `ряд «${row}»: контракт зовёт в ряд знак, которого на экране нет`,
      ).toEqual(
        ROW_PENDING.filter((item) => item.row === row)
  ...
  it("СТОРОЖ ОБРАТНОЙ ОШИБКИ ПРОВЕРЕН НА СЕБЕ: возвращаем `tab-room` в «угол»", () => {
    // СТОРОЖ, КОТОРОМУ НЕЧЕГО ЛОВИТЬ, ЗЕЛЁН ТАК ЖЕ, КАК СТОРОЖ, КОТОРЫЙ НИЧЕГО
    // НЕ ЛОВИТ. Проверка выше сегодня зелёная — и была бы зелёной, окажись
    // `rowComposition` пустышкой. Разница видна только на подложке, поэтому
    // подкладываем ровно тот файл, что приехал раундом 41: знак комнаты в углу
    // комнаты. Дизайн про свой такой же сторож написал: «первым он поймал бы
    // tab-room в углу» — вот это «первым» здесь и меряется.
    const drawn = drawnMeanings("угол", cornerRow);
    expect([...drawn].sort(), "экран угла перестал быть тремя знаками").toEqual([
      "friends",
      "settings",
      "treasury",
    ]);
    // На ЖИВЫХ данных чисто — иначе числа ниже ничего не значат.
    expect(rowComposition("угол", drawn).phantom).toEqual([]);

    const room = contract.set["tab-room.svg"] as IconEntry;
    // 1. Знак вернулся в БЛОК `rows`.
    const inBlock: IconsContract = {
      ...contract,
      rows: { ...contract.rows, угол: [...(contract.rows["угол"] as string[]), "tab-room.svg"] },
    };
    // 2. Знак вернулся в ПОЛЕ `rows` у знака, а блок чист. ВОТ ЭТОГО ВЧЕРАШНЯЯ
    //    ПРОВЕРКА НЕ ВИДЕЛА ВОВСЕ: она читала только блок.
    const inField: IconsContract = {
      ...contract,
      set: { ...contract.set, "tab-room.svg": { ...room, rows: ["бар", "угол"] } },
    };
    // 3. И оба списка разом — дословно раунд 41.
    const inBoth: IconsContract = { ...inBlock, set: inField.set };

    const caught = [inBlock, inField, inBoth].map(
      (forged) => rowComposition("угол", drawn, forged).phantom,
    );
    expect(caught, "сторож проспал возвращённый знак комнаты").toEqual([
      ["room"],
      ["room"],
      ["room"],
    ]);
    // ЧИСЛОМ: три подложки — три пойманных знака, ни одной тихой.
    expect(caught.flat()).toHaveLength(3);
    // И обратная сторона не путается с прямой: код рисует ровно то же, значит
    // `extra` на всех трёх подложках пуст.
    expect(
      [inBlock, inField, inBoth].map((forged) => rowComposition("угол", drawn, forged).extra),
    ).toEqual([[], [], []]);

    // ПРЯМАЯ СТОРОНА ТОЖЕ ЖИВА — вычеркнем из контракта знак, который экран
    // рисует. Вычёркивать надо из ОБОИХ списков: одного мало, второй вернёт.
    const treasury = contract.set["tab-treasury.svg"] as IconEntry;
    const dropped: IconsContract = {
      ...contract,
      rows: {
        ...contract.rows,
        угол: (contract.rows["угол"] as string[]).filter((f) => f !== "tab-treasury.svg"),
      },
      set: {
        ...contract.set,
        "tab-treasury.svg": { ...treasury, rows: treasury.rows.filter((r) => r !== "угол") },
      },
    };
    expect(rowComposition("угол", drawn, dropped).extra).toEqual(["treasury"]);
    expect(rowComposition("угол", drawn, dropped).phantom).toEqual([]);
  });
