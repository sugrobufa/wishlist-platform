...
describe("ИНВАРИАНТ-СЕРИАЛИЗАЦИЯ: страница хозяйки не знает о бронях вовсе", () => {
  it("owner-DTO всех зон при 3 бронях в БД: ни следа брони; счётчик отдельно равен факту", async () => {
    ...
    expect(
      ...
      hallOpen: true,
      // И не про брони: «у зрителя есть своя комната» — дорога домой из бара
      // гостя (тикет 253). У хозяйки ЭТОЙ комнаты она есть по определению.
      hasOwnRoom: true,
    });
  });
  ...
  it("вошла хозяйка этой комнаты → 200 с пустотой и no-store", async () => {
    ...
    expect(await response.json()).toEqual({
      ...
      data: {
        ...
        isOwner: true,
        hallOpen: true,
        // Тикет 253: «у зрителя есть своя комната» — по нему бар гостя зовёт
        // домой вместо «Собрать свою». У хозяйки ЭТОЙ комнаты она есть.
        hasOwnRoom: true,
      },
    });
  });
