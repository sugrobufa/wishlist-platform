...
const ROOM_KEYS = [
  ...
  "price",
  "priceVisibility",
  // «Где купить» в карточке вещи хозяйки (тикет 159, round39 → body.link).
  // Ключ есть у формы КОМНАТЫ всегда, значение `null` у вещи без адреса —
  // allowlist перечисляет форму, а не заполненность. У формы сокровищницы
  // ключа нет: там не покупают.
  "shop",
  "size",
  "title",
  "validUntil",
