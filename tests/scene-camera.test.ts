...
import { sceneZones, visibleZones } from "../src/components/scene/zones";
...
describe("наезд камеры (формула motion.json → openZone)", () => {
  ...
  it("масштаб наезда — формула motion.json, числа rooms.json в сцене не участвуют", () => {
    ...
    // Зоны шириной от 22 до 194 px — ровно то, из-за чего одно число дизайн
    // назвал ошибкой: узкой зоне нужно другое приближение, чем широкой.
    // 18 px были конверты `gamer/money` и `sport/money` — и они оказались НЕ
    // предметами: тикет 238 показал вырезками, что это точки РЯДОМ с банкой, а
    // не сами банки. По предмету копилки стали 34 и 40 px, и рекорд узости
    // перешёл `lux/jewelry` (22). То есть самой узкой зоной продукта два раунда
    // была разметка мимо цели, и приближение под неё считалось зря.
  ...
  it("телефон: правый край кадра теперь тоже обнажается — у 18 зон правой трети", () => {
    ...
    // Тикет 234 вернул восемнадцатую: `sport/watches` уехала со служебного
    // места посреди кадра на стеклянную шкатулку с часами, а та стоит у самого
    // правого края (556…630).
    // 15, а не 18: тикет 238 увёл из правого края все три копилки — их
    // прямоугольники стояли на 590, 590 и 577, то есть в 40 px от кромки кадра,
    // а сами банки стоят левее (557, 545, 558). Правый край у них больше не
    // обнажается, и обнажался он не из-за предметов, а из-за промаха разметки.
...
describe("позиция хотспота (доля кадра, rooms.json)", () => {
  const img = roomsContract.scene.phone.image;
  // Хотспот — это метка В КАДРЕ, поэтому зоны берутся списком СЦЕНЫ: живая
  // полка без места на кадре метки не получает вовсе (тикет 235).
  ...
  it("126 зон: доля кадра совпадает с картой (x/630, y/351) — без слагаемого", () => {
    // В контракте 134 адреса, меток в кадре 126: у восьми предмета в интерьере нет
    // (ADR-0006). С тикета 235 эти восемь живут двумя разными жизнями — часть
    // скрыта адресно, часть осталась живыми полками без места на кадре, — но
    // метки нет ни у одной, и для хотспота разницы никакой. Зона `money`
    // больше не вычитается — ADR-0008 включил её во всех десяти комнатах.
    // Прямоугольники всех 134 проверяет design-contract. Тикет 234 прибавил
    // четыре метки сразу: три предмета, найденных пакетом 50 глазами, и новую
    // полку «Бар и табак» — барную тележку в «Кабинете».
    //
