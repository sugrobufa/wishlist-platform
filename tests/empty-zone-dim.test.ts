...
//   1) правило «слой не множится на ночь» — иначе средняя ночь падает с 0.268
//      до 0.150, а самая тёмная комната с 0.200 до 0.112 (замер по кадрам,
//      Rec. 709, тем же способом, которым считались все числа про ночь);
...
/** Родные времена суток баз — их десять, и от них зависит правило ночи. */
const NATIVES = [...new Set(rooms.map((room) => room.tod))] as TimeOfDay[];
...
describe("рецепт пустой зоны — дословно из пакета", () => {
  ...
  it("к выбранному свету затемнение ДОБАВЛЯЕТСЯ, а не заменяет его", () => {
    expect(sceneFilter("dusk", "warm", false, "day", true)).toBe(
      `brightness(.8) saturate(1.08) ${EMPTY_ZONE_FILTER}`,
    );
  });
});

describe("слой не множится на ночь (правило дизайна, task19 → emptyZoneSceneClarify)", () => {
  it("выбрана ночь — второго затемнения нет", () => {
    expect(sceneFilter("night", "warm", false, "day", true)).toBe(
      gradingFilter("night", "warm", "day"),
    );
  });
  ...
  it("все десять комнат × четыре положения: ночь остаётся ровно ночью", () => {
    // Замеры ночи (0.268 средняя, 0.200 самая тёмная) держатся ровно на том,
    // что к ночному рецепту ничего не домножается. Умножение вернуло бы 0.150
