...
const SNAPSHOT: Record<string, [string, string, string]> = {
  ...
  // ДВЕ ТРОЙКИ ПЕРЕЕХАЛИ ТИКЕТОМ 238 — «Спорт» и «Лофт». Из десяти
  // переразмеченных им зон ЛЕВЕЕ окна правила пустых мест стояли ровно две:
  // `sport/tech` и `loft/grooming`. Остальные восемь правее и на выбор троек не
  // влияют вовсе — это и была причина не чинить их внутри тикета 233.
  //
  // В «Спорте» ноутбук 42×38 крупнее подноса с флаконами 49×29, поэтому среднюю
  // треть выиграл он: треть решается площадью ЗОНЫ, 1596 против 1421 (место
  // потом зажимается до 34 и различать зоны перестаёт — 34×34 против 34×29).
  //
  // В «ЛОФТЕ» ЗАПАС ВСЕГО 10 px², И ЭТО НАДО ЗНАТЬ. Несессер на кофре 50×37 =
  // 1850 обошёл ноутбук `tech` 46×40 = 1840 — первая треть сменила хозяина на
  // разнице в полпроцента. Оба места стоят на настоящих предметах, поэтому
  // выбор безразличен продукту; но следующая правка любого из двух
  // прямоугольников эту тройку перевернёт, и это будет не регрессия, а та же
  // арифметика. Ничья (равные площади) решалась бы порядком зон в rooms.json.
  sport: [
    "anything* 57x46@175,278 arm11 в-покое",
    "tech 34x34@268,193 arm11 в-покое",
  ...
  loft: [
    "grooming 34x34@196,245 arm11 в-покое",
