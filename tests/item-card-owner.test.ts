...
describe("числа контракта round45 — из контракта, а не набитые", () => {
  ...
  it("ФОРМА ПОДЛОЖКИ КРУГЛАЯ — и это единственное число не из `.imm-corner-mark`", () => {
    ...
    // РОДСТВО СОШЛОСЬ, И ПРОВЕРКА ПЕРЕВЁРНУТА ОБРАТНО (тикет 232, пакет 50).
    // Контракт зовёт круг «родством» со знаком витрины на кадре комнаты, а тот
    // был у нас КВАДРАТОМ — сутки здесь стояла проверка, ждавшая квадрата,
    // чтобы расхождение нельзя было закрыть молча; вопрос ушёл письмом 54.
    // Дизайн отменил свой вердикт раунда 49: знак угла — кнопка по его же
    // правилу, значит круг. Теперь строка требует совпадения: разойдутся формы
    // снова — покраснеет здесь.
    expect(condition(3).why).toContain("родство со знаком витрины");
    expect(value(ruleBody(globalsCss, ".imm-corner-mark"), "border-radius")).toBe("50%");
...
describe("чего мы из контракта НЕ взяли — ПЯТЬ ПРИЧИН ОТПАЛИ (round41)", () => {
  ...
  it("2. ВВОД подтверждён, а числа 6/5 сам же дизайн и снял (round49)", () => {
    ...
    // ХВОСТ ЗАКРЫТ, И ЗАКРЫЛ ЕГО ДИЗАЙН (тикет 252, пакет 55 → desire-guest,
    // турн 61c). Здесь стояло «6 px остались у показа — гостевой шкалы контракт
    // 49 не касается вовсе (открытый хвост тикета 225)», и записано это было
    // затем, чтобы «прибраться заодно» в чужой половине никто не пришёл молча.
    //
    // Никто и не пришёл: числа показа поехали не нашей уборкой, а его ответом
    // на замечание владельца «не видно, какие вещи любит хозяин». У показа
    // теперь 14 с шагом 10 — по той же причине, по которой у ВВОДА было 6/5
    // наоборот: «у гостя это НЕ цель нажатия, поэтому шага между целями 44
    // нет». Совпадение чисел показа и ввода кончилось окончательно, и обе
    // половины наконец описаны контрактом, а не одна из них.
    const scaleCss = read("../src/components/item/desire-scale.module.css");
    expect(scaleCss).toMatch(/\.flame \{[\s\S]*?width: 14px;/u);
    expect(scaleCss).not.toMatch(/\.flame \{[\s\S]*?width: 6px;/u);
