...
describe("светом отвечают ровно те зоны, у которых нет кадра «открыто»", () => {
  ...
  it("кадр «открыто» есть у двадцати двух зон продукта — им поведение не меняем", () => {
    ...
    expect(framed.map(({ room, zone }) => `${room}/${zone.key}`).sort()).toEqual(
      [
        ...
        "loft/travel",
        "lux/anything",
        // Восемнадцатая — тикет 81-2: кадр раунда 14 был честным, но мерился
        // против прямоугольника ниже жеста. Соседи переставлены — 0.93 → 3.31.
        "lux/travel",
        "study/anything",
        "warm/travel",
  ...
  it("свет достаётся всем остальным — тем самым мёртвым зонам из тикета", () => {
    ...
    // 126 МЕТОК В КАДРЕ (CLAUDE.md, инвариант №9: полок у продукта больше —
    // живые полки без места на кадре в кадр не выходят) минус 22 с кадром
    ...
    // Меток стало 126: тикет 234 вывел в кадр три предмета, найденных пакетом
    // 50 глазами, и новую полку «Бар и табак». Кадра «открыто» нет ни у одной
    // из четырёх, значит все четыре отвечают светом.
    expect(all.length).toBe(126);
    expect(waking.length).toBe(104);
  });
