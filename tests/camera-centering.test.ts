...
// 112 — `money` скрыта решением владельца (ADR-0004) и ещё восемь зон скрыты
// адресно: предмета нет в интерьере (ADR-0006).
//
...
//
// ТИКЕТ 40 СМЕНИЛ СИСТЕМУ КООРДИНАТ (прямоугольники переехали из окна 430 в
// кадр 630, ADR-0006), и это правка ровно того сорта, которую здешние пины и
// обязаны стеречь. Что важно: НИ ОДНО требование к центровке не сдвинулось —
// «зона приезжает в середину вьюпорта» держится с точностью до 9 знаков на
// всех зонах обеих раскладок, и контрактная формула по-прежнему совпадает с
// расчётом. Поехали только ПРОИЗВОДНЫЕ от карты числа — сколько пустоты
// открывает кадр и какие зоны не влезают в экран целиком: карта переехала, и
// эти статистики переехали с ней. Каждая такая правка ниже подписана причиной.
//
// А вот именной тест на промах прежней формулы стал ЖЁСТКИМ памятником: он
// больше не читает сегодняшнюю карту (см. комментарий внутри), потому что
// хранит замер приёмки, а замер не имеет права меняться задним числом.
...
describe("наезд доводит зону до центра (тикет 20)", () => {
  it("чистая функция совпадает с матричной моделью браузера (126 зон × 2 вида)", () => {
    // 134 в контракте − 8 (места на кадре нет, тикеты 235 и 234) = 126. Прежде
    // вычиталось ещё десять — зона `money` во всех комнатах; ADR-0008 включил
    // её. Тикет 234 прибавил четыре зоны сразу: три предмета, найденных пакетом
    // 50 (`warm/music`, `sport/watches`, `study/tech`), и новую полку
    // `study/bar` — барную тележку. Число зон, а не свойство расчёта: центровка
    // ниже проверяется на каждой из них поимённо.
    expect(allZones).toHaveLength(126);
  ...
  it("прежняя формула (с делением на scale) промахивалась на (S−1)·смещение", () => {
    ...
    /**
     * Прямоугольник `bold/music` В КООРДИНАТАХ СЦЕНЫ ТОГО ЗАМЕРА — вморожен
     * числом, и это важно.
     *
     * Здесь хранится ЗАМЕР ПРИЁМКИ: «к проигрывателю не приблизить», 145 px
     * промаха на телефоне и 91 на десктопе (ADR-0002, раздел «Контекст»).
     * Замер сделан по карте раунда 1, где зона стояла в 404,163 26×45 в
     * координатах ОКНА. С тех пор карту дважды переразметили — сегодняшняя
     * `bold/music` это 426,164 76×46 в координатах КАДРА, и та же арифметика
     * дала бы 170 px вместо 145.
     *
     * Если бы тест читал сегодняшнюю карту, число «145» тихо переписалось бы —
     * и документ, на который ссылается решение владельца, разошёлся бы с
     * тестом, который его якобы сторожит. Замер не имеет права меняться задним
     * числом, поэтому вход к нему приколочен, а не выведен.
     */
    const MEASURED_2026_08_05: Record<SceneView, { x: number; w: number }> = {
      // Телефонная сцена: карта раунда 1 задавалась прямо в её координатах.
      phone: { x: 404, w: 26 },
      // Десктопная: (x + 12) · 1.7778 — перевод «окно → кадр → десктоп» той
      // системы координат, с тем самым слагаемым, которого больше нет.
      desktop: { x: (404 + 12) * 1.7778, w: 26 * 1.7778 },
    };

    const missOf = (view: SceneView) => {
      const { w, h } = sceneSize(view);
      const scale = legacyScale[view];
      const r = MEASURED_2026_08_05[view];
      const c = viewportCenter(view);
      const zc = { x: r.x + r.w / 2 };
    ...
    // А починенный расчёт даёт ноль — и на сегодняшней карте тоже, для чего
    // берётся уже настоящая зона, а не вмороженная.
    const bold = must(
      rooms.find((room) => room.id === "bold"),
      "комната bold",
    );
...
describe("краевые зоны: сколько пустоты открывает кадр (решение ADR-0002)", () => {
  ...
  it("зажима смещения нет: худшая пустота — та, что даёт сам масштаб", () => {
    ...
    const worst = (view: SceneView) => {
      ...
      for (const { rect } of allZones) {
        ...
        right = Math.max(right, (g.right / w) * 100);
        top = Math.max(top, (g.top / h) * 100);
        bottom = Math.max(bottom, (g.bottom / h) * 100);
        if (g.left + g.right + g.top + g.bottom === 0) clean++;
      }
      return { left, right, top, bottom, clean };
    };
    // ПОЧЕМУ ЧИСЛА ПОЕХАЛИ (тикет 40, смена системы координат). Пустота у краёв
    // всегда была следствием того, ГДЕ на кадре стоит зона, а карта переехала:
    // 46 зон ушли в правую треть кадра, куда прежняя разметка не дотягивалась.
    // Отсюда две перемены, и обе ожидаемые:
    //   • появилась ПРАВАЯ пустота, которой раньше не было ни у кого. Прежде
    //     справа лежал нетронутый запас 200 px (кадр 630 против окна 430) —
    //     теперь там стоят зоны, и наезд на них обнажает правый край так же,
    //     как левый обнажался всегда;
    //   • чистых зон стало меньше (80 → 55 на телефоне, 69 → 42 на десктопе):
    //     зона у края кадра не может быть отцентрована без пустоты, а зон у
    //     края стало больше. Это геометрия пакета, а не расчёта.
    // Решение ADR-0002 (зажима нет) от этого не меняется: аргумент был про то,
    ...
    // Тикет 234: 63 → 65 на телефоне, 48 → 50 на десктопе. Прибавились ровно
    // две из четырёх новых зон — `warm/music` (проигрыватель на мраморной
    // консоли, 447…538) и `study/bar` (барная тележка, 425…483): обе стоят в
    // глубине кадра и центруются без пустоты. Две другие пустоту дают, но не
    // худшую: `sport/watches` упирается в правый край кадра (630), `study/tech`
    // — плёночная камера у левого (21). Пределы по всем четырём сторонам не
    // сдвинулись ни на десятую — худшие зоны прежние.
    // Тикет 238: 65 → 66 на телефоне, и ВЕРХНЯЯ ПУСТОТА УПАЛА 7.2 → 1.04.
    // Рекорд верхней пустоты держал `gamer/music`: его прямоугольник стоял на
    // 28 — почти у самой кромки кадра, — и наезд обнажал верх на 7.2%. Но в том
    // прямоугольнике аудиогруппы не было вовсе, это была пустая стена над ней
    // (вырезка 238-rects/before/gamer-music.jpg). По предмету зона встала на 77,
    // и худшей по верху стала другая. То есть 7.2 два раунда считалось ценой
    // разметки, а было ценой промаха.
    const phone = worst("phone");
    expect(phone.clean).toBe(66);
    expect(phone.left).toBeCloseTo(23.1, 1);
    expect(phone.right).toBeCloseTo(16.1, 1);
    expect(phone.top).toBeCloseTo(1.04, 1);
    ...
    expect(desktop.clean).toBe(50);
    expect(desktop.left).toBeCloseTo(30.5, 1);
    expect(desktop.right).toBeCloseTo(34.7, 1);
    // 5.51 вместо 7.2 — та же причина, что на телефоне: `gamer/music` ушёл с
    // пустой стены на аудиогруппу. На десктопе падение меньше, потому что кадр
    // вписан целиком и верхняя пустота считается от другой зоны.
    expect(desktop.top).toBeCloseTo(5.51, 1);
  ...
  it("зона целиком на экране — кроме высоких шкафов, где формула считает по ширине", () => {
    ...
    // она получает нижний упор формулы 1.45 и влезает. Взамен вылезают
    ...
    //
    // Тикет 40 переставил список, не меняя вывода: `emerald/home` после
    // переразметки стала широкой и низкой (67×136 → 52×28) и влезать начала, а
    // `emerald/books` — наоборот, выросла в 112×232 и перестала. Ровно тот же
    // случай «формула считает по ширине», только зона другая. Было:
    // cream/fashion phone · emerald/fashion phone · emerald/home phone ·
