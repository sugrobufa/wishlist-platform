...
describe("десктопный лист вещей вмещает ряд плиток целиком (тикет 60)", () => {
  ...
  it("телефонная ветка потолка не изменилась (тикет 59 остаётся в силе)", () => {
    ...
    //
    // Тикет 140 поменял в этой строке ОДНО слагаемое — то, на чём лист стоит:
    // `--sheet-bottom` вместо высоты чужой полосы. На десктопе оно равно
    // прежнему `--rail-bottom` (там под листом правда полоса), на телефоне —
    // высоте таб-бара. Само правило потолка («не выше нижней четверти кадра»)
    // не тронуто.
    expect(sceneCss).toContain(
      "max-height: calc(100dvh - var(--band-h) * 0.75 - var(--sheet-bottom));",
    );
    expect(sceneCss, "десктоп потерял прежнее основание листа").toMatch(
      /--sheet-bottom: var\(--rail-bottom\);/u,
    );
