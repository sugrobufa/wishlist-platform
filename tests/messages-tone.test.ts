...
// Правила тона переехали в модуль рядом (тикет 240): их читает ещё и сторож
// ПРОЗЫ пакета — `package-prose-tone.test.ts`. Правила не менялись ни буквой,
// переехал только их дом; разбор каждого стоит там же, где само правило.
import { CASE_TRAPS, GENDER_RULE, SHORT_LINE, TONE_RULES, caseTrap } from "./tone-rules";
 ...
 *   `Settings.light_warm` и `Settings.tod_dusk` — настоящие строки;
 ...
 *
 * `Hall.empty` ОТСЮДА УШЁЛ ТОЖЕ, И НЕ ПОТОМУ, ЧТО РАСХОЖДЕНИЕ ЗАКРЫЛОСЬ, — у
 * него сменилась ПРИЧИНА (тикет 257, решение владельца 16.08.2026). Строку
 * переписали заново, три дороги из неё убраны, и запись переехала в
 * `OWNER_REWRITE_257` ниже. Оставить её здесь значило бы держать замок с
 * разбором про перечисление дорог на строке, где никакого перечисления больше
 * нет: список объясняет, ПОЧЕМУ продукт говорит не как доска, и объяснение
 * обязано быть сегодняшним.
 */
const OWNER_REWRITE_89: readonly string[] = [];
 ...
 * подарили» и попал на экран с заголовком «Праздник ещё впереди» и кнопкой
 * «Праздник прошёл». Дословно: «Ничего абсолютно не понятно что происходит на
 * этой форме». Он прав дважды.
 *
 * 1. `notClosedHint` СОКРАЩЁН. Строка пакета говорила три вещи разом — про
 *    итог, про дату в настройках и про ручное закрытие, — потому что экран не
 *    различал состояний и одна подсказка обслуживала все. Теперь состояний три
 *    (итог открыт · праздник наступил · праздник впереди), и каждое говорит
 *    своё: про дату — `notClosedNoDate`, про ручное закрытие — `closeQuiet`.
 *    Остаток и есть строка «когда он пройдёт, здесь появится итог».
 *
 * 2. `closeButton` НАЗЫВАЛСЯ СОСТОЯНИЕМ, А НЕ ДЕЙСТВИЕМ. «Праздник прошёл» —
 *    это то, что человек СООБЩАЕТ экрану, а не то, что произойдёт по нажатию;
 *    рядом с заголовком «Праздник прошёл» кнопка повторяла его слово в слово.
 *    А произойдёт по нажатию необратимое: имена дарителей раскрываются один
 *    раз (инвариант №2), и действие обязано это называть.
 *
 * ЗАПРОС ДИЗАЙНУ — письмо 52: экран целиком (три состояния), слово кнопки и её
...
/**
 * ПЯТЫЙ список того же рода — решение владельца 14.08.2026 (тикет 242).
 *
 * `Hall.ownerHint` пакета: «В комнате — чего хочется. Здесь — что сбылось: эти
 * вещи не дарят». Слово владельца со скриншотом экрана сокровищницы: «Зачем
 * писать, что в комнате. Просто напиши, что тут хранятся вещи такие-то и
 * такие».
 *
 * Довод его сильный и виден на самом экране: человек СТОИТ в сокровищнице, над
 * строкой написано «Сокровищница · 26 вещей · уже твои», — а первая половина
 * подсказки объясняет ему устройство ДРУГОГО экрана. Про вещи, ради которых он
 * сюда зашёл, не сказано ничего.
 *
 * Наша редакция называет два источника вещей витрины (подарки и собственные
 * покупки — третьего нет: вещь попадает сюда либо через «Дошло», либо руками) и
 * сохраняет единственный факт, который тут нужен, — их не дарят и не бронируют
 * (инвариант №8).
 *
 * ЗАПРОС ДИЗАЙНУ — письмо 60. Приедет его редакция — запись обязана отсюда
 * уйти.
 */
const OWNER_REWRITE_242: readonly string[] = ["Hall.ownerHint"];

/**
 * ШЕСТОЙ список того же рода — решение владельца 16.08.2026 (тикет 257).
 * `Hall.empty`, пустая витрина.
 *
 * Строка пакета зачитывает ТРИ ДОРОГИ сюда: «Витрина пока пуста. Сюда
 * приезжают подарки после „Дошло" — и всё, что уже твоё: поставь вещь из зоны
 * или добавь прямо здесь». Слово владельца: «перепиши текст проще, а лучше
 * просто оставь что-то типа „Витрина пока пуста, добавь свою первую вещь",
 * плюс пояснение что здесь хранится».
 *
 * ЗАПИСЬ ПЕРЕЕХАЛА СЮДА ИЗ `OWNER_REWRITE_89`, где та же строка стояла с
 * 08.08.2026. Ключ тот же, расхождение то же, а причина другая: там наша
 * редакция ДОБАВЛЯЛА дороги, которых пакет не знал (их стало три вместо
 * одной), здесь она их убирает. Держать старый разбор на новой строке нельзя —
 * список объясняет, почему продукт говорит не как доска, и объяснение обязано
 * описывать сегодняшнюю строку.
 *
 * ДОВОД СИЛЬНЕЕ ВКУСА, И ОН ИЗ УСТАВА САМОГО ДИЗАЙНА: пакет 50
 * (`round50/empty-room.json` → `proposedInvariant`) просит записать инвариантом
 * «пустое состояние — ОДНО обещание и ОДНО действие; третий текст обязан снять
 * один из первых двух». Перечисление трёх дорог — инструкция, а не обещание, и
 * нарушало это правило раньше нас.
 *
 * ВТОРАЯ СТРОКА ПУСТОЙ ВИТРИНЫ (`Hall.emptyHint` — «здесь стоит то, что уже
 * твоё») правилу не противоречит: пустая сокровищница — ЕДИНСТВЕННОЕ
 * исключение его же памятки (`tone.md`: «там пусто по устройству продукта, и
 * это стоит объяснить, иначе человек будет искать кнопку»). Ключа этого пакет
 * не знает вовсе, поэтому его запись не здесь, а в списке сирот ниже.
 *
 * ЗАПРОС ДИЗАЙНУ — письмо 63, вместе с тем, что три дороги нарушали его же
 * правило. Там же сказано про «вещи, которыми гордишься»: строку он снял сам
 * (тикет 220, пакет 48 — «ставит человеку условие»), владелец просит вернуть, и
 * решает он. Приедет редакция дизайна — запись обязана отсюда уйти.
 */
const OWNER_REWRITE_257: readonly string[] = ["Hall.empty"];

/**
 * СЕДЬМОЙ список — И ЕДИНСТВЕННЫЙ, ГДЕ ПРИЧИНА НЕ РЕШЕНИЕ ВЛАДЕЛЬЦА, А ПАМЯТКА
...
const PACKAGE_ONLY: readonly string[] = [
  ...
  // `Settings.setOverline` — «Комнаты на выбор» (пакет 52). Настройка звалась
  // «набор зон», а это слово запрещено его же `tone.md` как техническое — и
  // вдобавок неверно: выбираются комнаты. Ключ ждёт своего тикета; заведём
  // вместе с показом числа комнат на плитках (58a), а не отдельной строкой без
  // экрана.
  "Settings.setOverline",
...
describe("словарь и дизайн-пакет", () => {
  ...
  it("ключи, которых пакет не знает, — только экраны входа по ссылке", () => {
    ...
    // комнату», «Потом» (турн 12c).
    //
    // ОТСЧЁТ ДО ПРАЗДНИКА ВЕРНУЛСЯ К РЕДАКЦИИ ДОСКИ (тикет 206). Доска писала
    // «День рождения через 12 дней», у нас стояло обезличенное «Праздник через
    // 12 дней», и причина была честной: комната хранила ДАТУ, но не знала, что
    // это за праздник, — обещать «день рождения» было бы враньём у половины
    // комнат. Тикет 198 дал ей это знание (день рождения, общие даты, свой
    // повод), 204 научил ленту, 206 — гостя. Строки получили `{holiday}`, имя
    // приезжает видом из сервиса, и расхождения с доской больше нет: у дня
    // рождения она теперь говорит ровно то, что доска и просила, а у Нового
    // года — «Новый год через 12 дней», чего доска не предусматривала вовсе.
    ...
    expect(own).toEqual([
      ...
      // Тикет 253: «Моя комната» — третий пункт гостевого бара, когда у зрителя
      // комната уже есть. Слова у дизайна нет: турн 56b рисовал ОДИН призыв
      // «Собрать свою», случай «гость с готовой комнатой» в баре не разбирался
      // вовсе. Наша нейтральная строка стоит, чтобы экран не врал человеку,
      // предлагая собрать то, что у него есть; слово и знак запрошены письмом
      // 63 и приедут пакетом.
      "GuestRoom.ctaMine",
      "GuestRoom.freeGifts",
      "GuestRoom.noSignup",
      ...
      // Тикет 251: «Позвать дарителя». Ключ ЕСТЬ у дизайна — он приехал
      // дельтой пакета 55 (`round55/strings-delta.json → added`), — но живой
      // `handoff/messages-ru.json` его ещё не знает: дельта не перевыпуск
      // словаря. Запись уйдёт отсюда, когда приедет словарь целиком, — ровно
      // как было с одиннадцатью строками карточки вещи из round44.
      "GuestRoom.shareRoom",
      // Тикет 247: «Комната {name}» — первый пункт гостевого бара. Ключ есть у
      // дизайна (дельта пакета 56), живой словарь его ещё не знает: дельта не
      // перевыпуск. Уйдёт отсюда вместе со словарём целиком.
      "GuestRoom.tabRoom",
      ...
      // `PACKAGE_ONLY` выше.
      "Hall.add",
      // Тикет 249: гостевая подпись витрины. Своей у неё не было — брался
      // ключ ХОЗЯЙКИНОЙ витрины, и гость читал под чужими вещами «уже моё»
      // («оно не моё, а его» — владелец, 14.08.2026). Пакет этого экрана не
      // знает по той же причине, что и остальные `Hall.guest*`: гостевого
      // маршрута не существовало, когда собирался словарь. Слово ушло письмом
      // 60 — приедет его редакция, ключ останется, значение сменится.
      "Hall.captionMineGuest",
      "Hall.deleteConfirm",
      "Hall.deleteNo",
      ...
      "Hall.edit",
      // ЧТО ЗА МЕСТО — ВТОРОЙ СТРОКОЙ ПУСТОЙ ВИТРИНЫ (тикет 257, решение
      // владельца 16.08.2026: «плюс пояснение что здесь хранится: уже ранее
      // купленные тобой вещи, которыми ты гордишься»). Ключ НАШ, пакет его не
      // знает: у него пустая витрина говорит одной строкой, и та зачитывает три
      // дороги сюда (запись о расхождении — в `OWNER_REWRITE_257` выше).
      //
      // Объяснение здесь законно по памятке самого дизайна: «единственное
      // исключение — сокровищница: там пусто по устройству продукта, и это
      // стоит объяснить, иначе человек будет искать кнопку». Первая строка при
      // этом остаётся обещанием с действием — правило пакета 50 («одно обещание
      // и одно действие») держится.
      //
      // «Вещи, которыми гордишься» дизайн снял у себя сам (тикет 220, пакет 48:
      // «ставит человеку условие, а сюда приезжает всё, что уже его, включая
      // подаренный крем»), владелец просит вернуть — решает он. Поэтому строка
      // называет ОБА источника, купленное самой и подаренное: подарки приезжают
      // сюда сами, после «Дошло», и врать про них она не должна. Ушло письмом
      // 63; приедет редакция дизайна — ключ останется, значение сменится.
      "Hall.emptyHint",
      ...
      "Hall.noteAdd",
      "Hall.noteQuote",
      // ЧТО ЭТО ЗА КОМНАТА — ОДНОЙ СТРОКОЙ (тикет 214, приёмка 13.08.2026).
      // Владелец: «В сокровищнице нужен небольшой поясняющий текст, что тут
      // хранятся вещи которыми ты гордишься и уже приобретены тобой». Гостю
      // такая строка у дизайна есть (`guestSubtitle` — «всё здесь уже дома»),
      // хозяйке нет: у пакета витрина хозяйки без подписи вовсе. Объяснение
      // здесь законно по его же памятке тона — «единственное исключение —
      // сокровищница: там пусто по устройству продукта, и это стоит
      ...
      //
      // 2. ТРИ ИМЕНИ ПРАЗДНИКОВ — ТЕПЕРЬ ЕГО ЖЕ ИМЕНАМИ (тикет 210, пакет 47 →
      //    `round47/strings-delta.json`). Просили прислать ключами — прислал, и
      //    ЗНАЧЕНИЯ СОВПАЛИ С НАШИМИ ДОСЛОВНО: «Новый год», «23 февраля»,
      //    «8 марта». Имена совпали на две трети, и две трети мы приняли:
      //    `holidayFeb23` совпал и именем, `holidayNewYear` → `holidayNY`,
      //    `holidayMarch8` → `holidayMar8`.
      //
      //    ПОЧЕМУ ЗДЕСЬ ПЕРЕИМЕНОВАНИЕ СТОИТ СДЕЛАТЬ, А В СЛУЧАЕ `Light.*`
      //    (раунд 43) НЕ СТОИЛО: тогда переезд восьми ключей в новую секцию
      //    давал ноль — сверка от него не менялась ни в одну сторону. Здесь
      //    имена ключей перестают быть НАШЕЙ догадкой о его словаре: разойтись
      //    им больше нечем, и следующая дельта ляжет на них без перевода. Это
      //    покупка, а не переезд.
      //
      //    СИРОТАМИ ОНИ ОСТАЮТСЯ, И ЭТО ЧЕСТНО: сверка «перенесено дословно»
      //    идёт с `handoff/messages-ru.json`, а его раздел `Occasion.*`
      //    праздников не знает вовсе (словарь собирали до раунда 44) — дельта
      //    же приезжает отдельным файлом. Приедет словарь с этими ключами —
      //    записи обязаны уйти отсюда, и замок «пакет знает, у нас нет» это
      //    заметит.
      //
      // 3. ЧЕТЫРЕ НАШИХ ПО ПАМЯТКЕ И ДВА ОТСЧЁТА ОТ ДИЗАЙНА:
      //    - `leftDays` — «{days} день/дня/дней» для `{left}` в `offerTitle`.
      //      Был наш, назывался `offerLeft`; пакет 47 прислал его своим именем
      //      и своим значением — значение совпало слово в слово, имя принято
      //      (тикет 210);
      //    - `leftWeeks` — «{weeks} неделю/недели/недель», НОВЫЙ ключ того же
      //      пакета. Плашка приходит РОВНО за три недели (`OFFER_LEAD_DAYS`) и
      //      в первый свой день говорила «Через 21 день»; дизайн: «предложение
      //      приходит именно за три недели, и „через 21 день" там читается
      //      хуже». Употребление одно и настоящее — та же плашка: целые недели
      //      говорятся неделями (`holidays.wholeWeeks`), остальное днями.
      //      Заводить ключ без употребления было нельзя: сирота без места в
      //      интерфейсе возвращается в UI как «готовая строка» (прецеденты
      ...
      // противоречие, на которое владелец и наткнулся: комната зовёт «Праздник
      // прошёл», экран отвечает «Праздник ещё впереди».
      //
      // `dueTitle` / `dueHint` — праздник НАСТУПИЛ, итога ещё нет (то самое
      // состояние, которого у пакета не было). `closeQuiet` — тихая дорога к
      // ручному закрытию для комнаты, где праздник впереди: громкой кнопке
      // там не место, она горела бы до следующего года (слова владельца:
      ...
      // `Settings.light_candle` ОТСЮДА УШЁЛ (тикет 256, приёмка владельца
      // 16.08.2026): «Свечной свет и Тёплый непонятно чем отличается, я не
      // заметил на комнате. Если разницы нет, то оставь только один из них —
      // Тёплый». Положение снято целиком — тем же приёмом и тем же доводом,
      // каким уходила «ночь» строкой ниже: два одинаковых положения с разными
      // именами это ложь интерфейсу. Пара строк удалена из словаря вместе с
      // ним; пакет их по-прежнему ЗНАЕТ (`Light.colorCandle` и
      // `Light.stateCandle` — записи в `PACKAGE_ONLY` выше).
      "Settings.light_warm",
      ...
      "Settings.roomState",
      // `Settings.state_candle` («свечной свет» в строке под кадром) ушёл
      // вместе со своим положением — тикет 256, разбор у `light_candle` выше.
      "Settings.state_day",
      "Settings.state_dusk",
