 ...
 * их различать:
 * - `Hall.remove` — «Убрать с витрины» из пакета не годилось: рядом появился
 *   глазок, который тоже «убирает с витрины», только у гостей. Тикет 89 дал
 *   «Убрать из сокровищницы», тикет 123 (решение владельца 09.08) — нынешнее
 *   **«Вернуть в комнату»**: вещь попадает на витрину и по ошибке, и дорога
 *   назад обязана называться дорогой назад, а не вторым удалением. Дизайн это
 *   слово принял (турн 36b), но своего `messages-ru.json` в репозиторий не
 *   присылал — расхождение с пакетом осталось прежним по форме;
 ...
 *
 * ЗАПРОС ДИЗАЙНУ: экран витрины с тремя действиями он нарисовал (36b), но
 * словарь пакета в репозитории — прежний. Приедет обновлённый
 * `handoff/messages-ru.json` — эти записи обязаны отсюда уйти.
 */
...
describe("словарь и дизайн-пакет", () => {
  it("каждая строка пакета перенесена дословно — кроме записанных расхождений", () => {
    const allowed = new Set([...PACKAGE_DRIFT, ...OWNER_REWRITE_89, ...OWNER_REWRITE_105]);
    const drift = [...handoff]
  ...
  it("ключи, которых пакет не знает, — только экраны входа по ссылке", () => {
    ...
    expect(own).toEqual([
      ...
      // Подстроки листа «⋯» (тикет 123, 36b): «знак + титул + подстрока».
      // Слова с макета дословно — «спросим ещё раз», «останется у тебя на
      // витрине»; `showHint` — обратное положение глазка, макет рисует одно.
      "Hall.deleteHint",
      "Hall.deleteNo",
      "Hall.deleteYes",
      ...
      "Hall.priceSeenAria",
      "Hall.priceShow",
      // Подсказка «Вернуть в комнату»: называет зону по имени и напоминает про
      // цену — «встанет в свою зону — в „{зона}“, цена снова видна» (турн 36b,
      // тикет 123). Прежняя половина строки пришла поставкой раунда 19, вторая
      // («цена снова видна») — этим раундом: вещь уезжает с витрины туда, где
      // цена показывается по `priceVisibility`, и человек вправе знать это ДО
      // нажатия, а не после.
      "Hall.removeHint",
      "Hall.seenALL",
      "Hall.seenFRIENDS",
