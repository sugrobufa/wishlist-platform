...
describe("словарь и дизайн-пакет", () => {
  ...
  it("ключи, которых пакет не знает, — только экраны входа по ссылке", () => {
    ...
    expect(own).toEqual([
      ...
      // ПЕРЕПИСАНА ПОД ПРАВИЛО РОДА (тикет 172). Было «Гость увидит её такой,
      // какой ты выбрала» — родовая форма, обращённая к человеку, и правило
      // выше ловит её первой же. Стало «У гостя она откроется с этим же
      // светом»: то же обещание назывным оборотом, и оно не ломается, если у
      // комнаты хозяин. Строка наша, в словаре пакета её нет — сверять не с
      // чем, поэтому запись живёт здесь, а не в списке расхождений.
      "Settings.lightHint",
      "Settings.lightOverline",
      "Settings.light_candle",
      "Settings.light_warm",
      "Settings.light_white",
      // `Settings.studioApplied` / `studioPending` — подписи крупного кадра в
      // настройках (тикет 181). Экрана в пакете нет: он заказан дизайну
      // письмом 46 и ещё не приехал. Слова наши и по памятке — назывное «Так
      // комната и выглядит» и честное «ещё не в комнате», потому что превью
      // показывает выбранное, а вещи по полкам двигает только «Переехать».
      // Приедет его редакция — строки уйдут отсюда в обычную сверку.
      "Settings.studioApplied",
      "Settings.studioPending",
