...
describe("словарь и дизайн-пакет", () => {
  ...
  it("ключи, которых пакет не знает, — только экраны входа по ссылке", () => {
    ...
    expect(own).toEqual([
      ...
      // «видишь только ты», «спросим ещё раз». Пары к ним — `itemShowHint` и
      // `itemHallRemoveHint` — обратные положения тех же двух строк: макет
      // рисует лист один раз, а вещь бывает и спрятанной, и уже на витрине.
