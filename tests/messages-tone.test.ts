...
describe("словарь и дизайн-пакет", () => {
  ...
  it("ключи, которых пакет не знает, — только экраны входа по ссылке", () => {
    ...
    expect(own).toEqual([
      ...
      "TabBar.settings",
      "TabBar.tabsAria",
      // Сироты по убыли, третья тройка — `ZoneGrid._v2`: «состояний больше
      // нет. Ряд фильтров „Люблю/Хочу" снят без замены (хозяйке)», ключи
      // tabsAria/tabLove/tabWant/emptyLove/emptyWant/loveCaption сняты.
