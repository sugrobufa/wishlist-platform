...
//
// ЭТОТ БЕРЕГ ТЕПЕРЬ ДАЛЬНИЙ (тикет 237). Замечание владельца родителем не
// закрылось: последние 265 px кадр ехал один — ручки уже за его спиной, — и
// ближний берег ставит наблюдатель пересечения (`knobs-in-sight.ts`,
// `tests/settings-frame-lets-go.test.ts`): липкость снимается, как только ушла
// последняя ручка. Проверки НИЖЕ остаются в силе и обязаны быть зелёными:
// наблюдателя в старом браузере может не быть, а выйти за свой блок кадр не
// должен никогда.
