...
describe("143 — низ поверхности задаёт страница, а не раскладка", () => {
  ...
  it("МЕСТО ПОД БАР РЕЗЕРВИРУЕТ ТОТ, КТО БАР РИСУЕТ — теперь обе страницы", () => {
    // ЭТОТ ТЕСТ ЗДЕСЬ И УПАЛ, КАК ОБЕЩАЛ. Он держал правило 143 «у гостя
    // таб-бара нет и быть не должно» и прямо предупреждал: «появится бар у
    // гостя — этот тест обязан упасть». Он появился тикетом 247, и появился по
    // решению владельца и дизайна, а не по недосмотру: призыв «Собрать свою»
    // лежал строкой ниже списка зон и был недостижим — список растёт с числом
    // полок (турн 56b, пакет 55). Правило 143 отменено не целиком: связь
    // осталась той же, изменилось число рисующих.
    //
    // Связь: место под бар резервирует ТОТ, КТО БАР РИСУЕТ. Обе страницы
    // рисуют — значит ни одна не объявляет своего `--imm-bar-h`, и обе берут
    // дефолт `--imm-tab-bar`. Он уже включает нижний инсет (тикет 182), и
    // прибавлять его второй раз нельзя ни той, ни другой.
    expect(ownerPage, "комната хозяйки перестала рисовать бар").toMatch(/<TabBar[\s\S]*?phoneOnly/u);
    expect(guestPage, "гость перестал рисовать свой бар").toMatch(/<GuestBar[\s\S]*?roomHref/u);
    for (const [who, page] of [
      ["хозяйка", ownerPage],
      ["гость", guestPage],
    ] as const) {
      expect(page, `${who} объявила своё число — дефолт стал мёртвым`).not.toContain(
        '"--imm-bar-h"',
      );
    }
  });

  it("у гостя в баре нет «Добавить» — в чужую комнату вещь не положишь", () => {
    // Бары у них разные по составу, и это не косметика: «Добавить» ведёт в
    // форму создания вещи В СВОЕЙ комнате. Гостю она недоступна по смыслу.
    const guestBar = read("../src/app/r/[slug]/guest-bar.tsx");
    expect(guestBar).not.toContain("ADD_HREF");
  });
});
