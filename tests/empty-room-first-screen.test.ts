...
describe("блок помещается: числа замера, а не эскиза", () => {
  ...
  it("блок не починен уменьшением своих чисел", () => {
    ...
    // Просветы блока сюда больше не входят — их стережёт разбор контракта ниже
    // (тикет 255): одного числа на все три там нет и быть не может.
...
// ---------- Тикет 255: просветов внутри блока ТРИ, и они разные ----------
//
// ПРИЁМКА 16.08.2026: «кнопка „Добавить первую вещь"… прилипла к тексту выше».
// Замечание про вид кнопки ушло дизайну письмом, а отступ оказался нашей
// поломкой: `fit.breakdown` пакета 44 разбирает блок по пикселям и называет три
// РАЗНЫХ просвета — 12 над заголовком, 10 над строкой, 22 над кнопкой, — а в
// коде стоял один общий `gap: 10px` на всё. Кнопка недобирала ровно 12 px там,
// куда владелец и показал пальцем: расхождение с пакетом, то есть баг.
//
// ЧИСЛА ЧИТАЮТСЯ ИЗ САМОГО КОНТРАКТА. Переписать их сюда руками значило бы
// завести вторую правду: пакет однажды передумает, а зелёный сторож будет
// сверять код с устаревшим числом и молчать.
const contract = JSON.parse(read("../design/package/handoff/round44/empty-room.json")) as {
  fit: { breakdown: string; measuredFrom: string };
};

/** Слагаемые разбора по порядку: «22 отступ», «9 надстрочная», «12», … */
const TERMS = contract.fit.breakdown.split("+").map((term) => term.trim());

/**
 * ПРОСВЕТ — ГОЛОЕ СЛАГАЕМОЕ РАЗБОРА, а чей он — говорит слагаемое СЛЕДУЮЩЕЕ:
 * «… + 12 + 58 заголовок …» значит «12 над заголовком». Разбирать так, а не
 * брать третье число по счёту, — чтобы сторож упал со словами, если пакет
 * перепишет разбор, вместо того чтобы уверенно сверить не тот просвет.
 *
 * Слагаемое роста — «<число> <кого>» и дальше что угодно: пакет дописывает то
 * пояснение («58 заголовок (две строки)»), то сумму («19 кнопка = 191»).
 */
const named = (term: string): string => /^\d+\s+(\S+)/u.exec(term)?.[1] ?? "";

const gapBefore = (word: string): number => {
  const at = TERMS.findIndex((term) => named(term) === word);
  expect(at, `в fit.breakdown нет слагаемого «${word}»: ${contract.fit.breakdown}`).toBeGreaterThan(
    0,
  );
  const before = TERMS[at - 1] ?? "";
  expect(before, `перед «${word}» в разборе стоит не просвет, а «${before}»`).toMatch(/^\d+$/u);
  return Number(before);
};

/** Тело базового (телефонного) правила: в ветке ≥1024 селекторы с отступом. */
const baseRule = (selector: string): string => {
  const at = globalsCss.indexOf(`\n${selector} {`);
  expect(at, `базового правила ${selector} в globals.css нет`).toBeGreaterThan(-1);
  return globalsCss.slice(at, globalsCss.indexOf("}", at));
};

describe("три просвета блока пустой комнаты — числа контракта", () => {
  it("разбор контракта даёт ровно три просвета, и они разные", () => {
    // Смысловая часть, без которой числа ниже — просто числа. Стало бы их два
    // или все три одинаковыми — общий `gap` вернулся бы законно, и падать
    // должно здесь, а не на CSS.
    const gaps = TERMS.filter((term) => /^\d+$/u.test(term)).map(Number);
    expect(gaps, `голых слагаемых в разборе не три: ${contract.fit.breakdown}`).toHaveLength(3);
    expect(new Set(gaps).size, "просветы контракта перестали быть разными").toBeGreaterThan(1);
  });

  it("общего `gap` у блока нет: три разных числа им не выражаются", () => {
    // Ровно та поломка, которую чинил тикет 255. Одно число на всё выглядит
    // аккуратнее и потому возвращается само собой — при первой же уборке.
    expect(
      baseRule(".imm-empty-start"),
      "у блока снова общий `gap` — значит, три просвета опять стали одним",
    ).not.toMatch(/gap:/u);
  });

  it("12 над заголовком, 10 над строкой, 22 над кнопкой", () => {
    expect(baseRule(".imm-empty-title")).toMatch(
      new RegExp(`margin:\\s*${gapBefore("заголовок")}px 0 0;`, "u"),
    );
    expect(baseRule(".imm-empty-body")).toMatch(
      new RegExp(`margin:\\s*${gapBefore("строка")}px 0 0;`, "u"),
    );
    expect(baseRule(".imm-rail .imm-empty-cta")).toMatch(
      new RegExp(`margin-top:\\s*${gapBefore("кнопка")}px;`, "u"),
    );
  });

  it("надстрочной просвет не положен: 22 из разбора — снаружи блока", () => {
    // Первое слагаемое «22 отступ» отмеряно ОТ КРОМКИ КАДРА (`fit.measuredFrom`),
    // то есть до блока, а не внутри него: держит его стопка слота `below`.
    // Поставить те же 22 ещё и первому ребёнку значило бы отсчитать их дважды.
    expect(contract.fit.measuredFrom).toContain("от кромки кадра");
    expect(TERMS[0]).toMatch(/^22\s+отступ$/u);
    expect(baseRule(".imm-empty-places"), "надстрочная обзавелась своим отступом").not.toMatch(
      /margin/u,
    );
  });
});
