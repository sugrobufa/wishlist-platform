...
describe("гостевой фильтр «только свободные»", () => {
  ...
  // ПЕРЕПИСАНО (тикет 124): «и только среди „хочу"» проверять нечем —
  // состояний нет, а в списке комнаты лежат только её вещи. Фильтр остался
  // ровно про брони, и это единственное, что он теперь и должен делать.
  it("фильтрует ровно по отсутствию брони", () => {
    expect(view).toMatch(/items\.filter\(\(item\) => !takenIds\.has\(item\.id\)\)/u);
  });
