...
// Словарь настоящий и здесь: слова экрана сверяются с тем, что в нём написано,
// а не с копией в тесте. Тем же файлом накрыт `next-intl` ниже.
 ...
 * исходника: числа, форму и место подложки держит `tests/item-card-owner.test.ts`
 * (с тикета 221 она КРУГЛАЯ, и тени под ней нет), а увидеть, включилась ли она,
 * можно только в разметке. Случая ровно два, и второй — тот, ради которого
 * заведён класс: у вещи БЕЗ фотографии знаки стоят на собственной заливке .05,
 * и тёмный круг был бы там пятном — а тень остаётся их единственной защитой.
 */
describe("знаки на фотографии получают подложку (тикет 218)", () => {
  ...
  it("ПУСТАЯ ШКАЛА НА ЭКРАНЕ ГОВОРИТ «не сказано», а не спрашивает (тикет 221)", () => {
    // Здесь ВЫЗОВОМ: слово выбирает карточка, а увидеть, какое из двух доехало
    // до экрана, можно только в разметке. В форме шкала — вопрос, в карточке —
    // ответ (пакет 48 → round48/desire-scale.json).
    const empty = owner(dbItem({ desire: null }));
    expect(empty).toContain(ru.ItemCard.desireEmpty);
    // ГЛАЗАМ вопроса формы на экране нет. Читалке он остаётся ИМЕНЕМ ГРУППЫ —
    // `aria-label` стоит там с раунда 29 и называет сам орган ввода, а не его
    // значение; поэтому он вычёркивается, а не ловится.
    expect(
      empty.replace(/aria-label="[^"]*"/gu, ""),
      "в карточку вернулся вопрос формы",
    ).not.toContain(ru.AddItem.desireLabel);
    // Со степенью на том же месте — слово ступени, и подпись молчит.
    const chosen = owner(dbItem({ desire: 3 }));
    expect(chosen).toContain(ru.AddItem.desire3);
    expect(chosen).not.toContain(ru.ItemCard.desireEmpty);
    // У вещи ВИТРИНЫ шкалы нет вовсе — желание исполнено, и слова тоже нет.
    expect(owner(dbItem({ inHall: true }), null)).not.toContain(ru.ItemCard.desireEmpty);
  });
