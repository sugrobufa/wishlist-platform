...
- **Изображения комнат неприкосновенны.** 84 хотспота привязаны к композиции
...
- **Наезд ≠ раскрытие.** Наезд бесплатен и работает для всех 84 зон (та же
  фотография). Раскрытие — кроссфейд в отдельный кадр «открыто» (30 кадров,
  по три на комнату); у зоны без кадра камера подъезжает, сетка вещей
  работает, мебель не двигается.
...
- **`itemForOwner`** — НИКОГДА не содержит `booking`, `guestName`, ничего о
  ...
  не по вещам.
- Исключение — складчина ≥5 участников: «её не спрятать» (точное поведение —
  открытый вопрос PRD §12.6; до уточнения хозяйке отдаётся факт складчины без
  имён).
