...
/**
 * «Сокровищница» — арка со скважиной (набор раунда 21, доска Б19).
 *
 * ЗАМЕНИЛА ДВОЙНУЮ ЗВЕЗДУ. Звезда рисовалась под старое имя раздела — «зал
 * славы», — а с переименованием (решение владельца 06.08.2026, тикет 62)
 * перестала называть то, что называет. Новый знак рифмуется с двумя вещами
 * сразу: аркой «Комнаты» (тот же силуэт, только закрытый) и знаком продукта
 * G — скважина повторяет его прорезь.
 *
 * Толщина вернулась к общей 1.7: авторские 1.6 были у звезды, у арки набор
 * держит единый контур.
 *
 * МЕСТА В ТАБ-БАРЕ У НЕГО БОЛЬШЕ НЕТ (тикет 119): вход в витрину переехал
 * знаком-шкатулкой в угол сцены (`IconTreasury` ниже, файл `ui-treasury.svg`).
 * Знак остаётся здесь как часть КАНОНИЧЕСКОГО НАБОРА пакета — файл описывает
 * набор доски, а не список того, что сейчас на экране, и `tab-icons.test.ts`
 * продолжает сверять его с `tab-treasury.svg` путь в путь. Новый вход
 * рисовать этим знаком нельзя: в углу стоит шкатулка, и двух разных знаков
 * одного места быть не должно.
 */
export function IconHall(props: IconProps) {
  return (
    <Icon24 {...props}>
      <path d="M4.5 11.5V11a7.5 7.5 0 0 1 15 0v.5" />
      <path d="M4.5 11.5h15V19a1.5 1.5 0 0 1-1.5 1.5H6A1.5 1.5 0 0 1 4.5 19v-7.5z" />
...
/**
 * «Сокровищница» — шкатулка (набор раунда 24, турн 36c;
 * `handoff/icons/ui-treasury.svg`).
 *
 * Второй знак угла сцены (тикет 119). Крышка чуть приподнята — тонкая линия
 * света в щели, — и скважина рифмуется со знаком продукта G: шкатулка
 * «открывается», как всё в комнате.
 *
 * НЕ ПУТАТЬ С `IconHall`: тот — арка со скважиной из набора таб-бара
 * (раунд 21), и места в баре у витрины больше нет.
 */
export function IconTreasury(props: IconProps) {
  return (
    <Icon24 {...props}>
      <path d="M4.5 12.5V12c0-4.1 3.4-7.5 7.5-7.5s7.5 3.4 7.5 7.5v.5" />
      <path d="M3.5 12.5h17" />
      <path d="M4.5 12.5H19.5V19a1.5 1.5 0 0 1-1.5 1.5H6A1.5 1.5 0 0 1 4.5 19v-6.5z" />
      <path d="M7.2 9.8c.4-1.9 2-3.3 4-3.5" />
...
// `icons/action-*.svg`, и четыре из них — ДРУГАЯ геометрия, чем у знаков того
// же смысла из канона 25a: у карандаша другой наклон, у корзины крышка на
// полпикселя выше, у шкатулки уже арка. Переписать канон нельзя — те знаки
// живут в таб-баре и в углу сцены и сверяются с `tab-*.svg` (tests/tab-icons);
// нарисовать «похоже» тоже нельзя (правило набора). Поэтому знаки строки вещи
// — свои компоненты, сверенные со своими файлами (tests/item-actions).
//
...
/** «В сокровищницу» — шкатулка со скважиной, узкий вариант листа действий. */
export function IconActionTreasury(props: IconProps) {
  return (
    <Icon24 {...props}>
      <path d="M5 11.5V11a7 7 0 0 1 14 0v.5" />
      <path d="M5 11.5h14V19a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 5 19v-7.5z" />
