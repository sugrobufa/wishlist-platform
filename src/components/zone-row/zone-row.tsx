...
// СВОЕГО КЕГЛЯ У ЗНАКА «⋯» ЗДЕСЬ НЕТ. Полгода строка носила собственное число
// 20 (`ZONE_ROW_GLYPH`) — и оно было ОПИСКОЙ пакета, подтверждённой дизайном
// письмом 42: «глиф ⋯ — 19, как весь набор». Разницу в пиксель с соседней
// строкой витрины поймал глаз, а не тест. Теперь кегль общий — `SIGN_SIZE`
// (item-actions), и второго числа на тот же знак завести негде.
...
export type ZoneRowProps = {
  ...
  hidden?: boolean;
  /**
   * Знак «⋯» в конце. У гостя его нет вовсе — тогда имени +38 px: коробка 32
   * (контракт → form.trailing.box) плюс промежуток 6 перед ней.
   */
  trailing?: ReactNode;
