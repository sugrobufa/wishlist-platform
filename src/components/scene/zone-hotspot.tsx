 ...
 * прямоугольник остался невидимой областью нажатия того же размера, а от него
 * живут три слоя — пятно, тень-кольцо и точка-искра. В покое видна только
 * искра; свечение разгорается по приближению указателя — силу пишет движок
 * близости SceneStage через `--zone-near` (тикет 50). Четвёртый слой метки
 * (затемнение остального кадра) живёт выше, на уровне сцены.
 */
export function ZoneHotspot({
  ...
  return (
    ...
    >
      ...
      {/* Слоя локальной тени (vignette) больше нет: на светлой комнате он
          читался тёмным кругом вокруг предмета — вторая жалоба владельца на
          то же место. Затемнение теперь одно, общее: .dim на уровне сцены,
          сила — от близости указателя (--zone-near-max). */}
