 ...
 * молча.
 *
 * «РАМКИ ПРЕДМЕТОВ» ПРИЁМКИ 14.08.2026 (замечание 3, тикет 230) — ЭТО НЕ КОД.
 * Владелец увидел в пустом «Кабинете» два уголка, висящих в воздухе у стола и
 * кресла. Смотрели глазами по вырезке, а не тестом (правило проекта; вырезки —
 * `.scratch/acceptance-2026-08-14/230-places/`), и правило с формулой оказались
 * ни при чём: все 30 мест сошлись с `receipt` пакета до пикселя, тройки тоже.
 * Мимо предмета едет сам ПРЯМОУГОЛЬНИК ЗОНЫ, ровно в трёх мужских комнатах:
 * `gamer/sport` (370,164,80,56 — голая стена; прежний `rectOld` 375,214,55,72
 * стоял на скамье с гантелью и кроссовками), `sport/grooming` (200,170,60,34 —
 * стена слева от зеркала, поднос с уходом на 30 кадр-px ниже) и `study/sport`
 * (300,244,68,86 — пол под столом: в кадре раскрытия `o-study-sport.jpg`
 * гантели ВЫНУТЫ из стойки и стоят ровно там, то есть прямоугольник размечен по
 * кадру «открыто», а не по базовому). Лечится переразметкой этих трёх зон —
 * письмо дизайну; здесь менять нечего, и подвинуть место своим числом нельзя:
 * второй карты координат не существует (CLAUDE.md).
 */
// `@design/places.json` — ЖИВОЙ контракт мест под стабильным именем; в нём
// лежит `places-v3.3` раунда 41 ПЛЮС точечная дельта раунда 42 (тикет 178):
// пять строк, четыре из них — доложенные числа псевдонимов. Присланные пакеты
// хранятся рядом неприкосновенными (`handoff/round41/places-v3.3.json` — база
// дельты, `design/round42/places-delta.json` — сама дельта), и расходятся файлы
// теперь ровно на эти пять строк: архив на то и архив, чтобы по нему сверять.
 ...
 *
 * `behavior.a11y` НЕ ИЗМЕНИЛСЯ: шесть ключей те же побайтно, `role`/`name`/
 ...
 *
 * ЧТО ПРИВЁЗ РАУНД 42 (тикет 178) — ЧЕТЫРЕ ЧИСЛА И ОДНУ ПРАВЛЕНУЮ ЗАПИСЬ.
 * `states.hover.stroke` / `states.zoneHover.stroke` = 1.5 и
 * `states.press.opacity` / `states.zonePress.opacity` = 1: числа не новые (1.5
 * — базовый `visual.corners.strokePx`, 1 — непрозрачность, которая при нажатии
 * не меняется), но псевдоним обязан отдавать то же, что старый адрес. Читаем мы
 * их по-прежнему НЕ ОТСЮДА, и рендер от переноса не сдвинулся ни на пиксель.
 * А строка `keyMoves.fromV31ToV33 → behavior.a11y` была ЛОЖНОЙ: она обещала
 * «значение изменилось», побайтная сверка показала обратное, и дизайн её
 * переписал — «значение НЕ менялось + добавлен псевдоним».
 *
