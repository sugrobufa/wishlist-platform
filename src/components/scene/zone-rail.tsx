...
export function ZoneRail({
  ...
  return (
    <div className={active ? `${s.stack} ${s.stackAway}` : s.stack}>
      {/* В ПУСТОЙ КОМНАТЕ СТРОКА ДЕЙСТВИЙ ПУСТА (тикет 199): «Добавить вещь»
          здесь только на десктопе, «поделиться» живёт после вещей, знака
          «Списком» нет — переключать нечего. Метка снимает с неё резерв под
          цель нажатия (globals.css → .imm-row-bare); те 19 px и были местом,
          которого блоку первого шага не хватало на 390×664.
          С тикета 208 метка снимает и вторые 25: пилюля-обещание, под которую
          строка держала зазор, уехала в кадр, и резервировать в полосе больше
          нечего. */}
      <div className={empty ? "imm-row imm-row-bare" : "imm-row"}>
