...
export function SceneStage({
  ...
  /**
   * Внутренний слой хотспотов — тот, что везёт покой кадра (тикет 259). Движок
   * близости меряет клиентскую точку ОТ НЕГО, а не от внешней коробки: перевод
   * «точка пальца → координата кадра» обязан идти через тот же трансформ, что
   * и сама фотография, иначе свет разгорается не у той зоны, на которую человек
   * смотрит. Коробка дышащего слоя возвращается уже с текущей фазой дыхания —
   * ровно то, что нужно.
   */
  const hotspotsFrameRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    // Меряем от ДЫШАЩЕГО слоя (тикет 259): он повторяет покой кадра, и его
    // коробка приходит с текущей фазой дыхания. Внешняя коробка стоит на месте
    // и до тикета 259 давала перевод, расходившийся с картинкой до 9% ширины.
    const layer = hotspotsFrameRef.current ?? hotspotsLayerRef.current;
    ...
    >
      <div ref={viewportRef} className={zoomedIn ? `${s.viewport} ${s.zoomed}` : s.viewport}>
        ...
        <div
          ...
          {...zoneEvents}
        >
          {/* МЕТКИ И УГОЛКИ ПОВТОРЯЮТ ПОКОЙ КАДРА (тикет 259, приёмка
              16.08.2026: «уголки не попадают в вещи и всё время гуляют при
              передвижении комнаты»).

              Слой хотспотов лежит СНАРУЖИ стопки камеры — так и задумано: при
              наезде он скрыт, и трансформы наезда ему не нужны. Но снаружи он
              оказался и у двух трансформов ПОКОЯ, которые кадр везёт всегда:
              вес камеры `scale(1.02)` (--lead-rest) и дыхание
              `scale(1.10 → 1.13) translate(±1.1%)`. Фотография в покое
              нарисована примерно в 1.12 раза крупнее коробки, по которой мы
              расставляем метки, и медленно ходит маятником — отсюда обе
              половины жалобы разом: уголок стоит не на предмете и не стоит на
              месте.

              Это незакрытое «Следствие 2» ADR-0002 от 05.08.2026, с замером
              тогда же: (дыхание − 1) · scale · расстояние до центра ≤ 8.1%
              ширины плюс собственный ход ±1.9%; живой замер по трём комнатам —
              9.14% ширины окна (телефон 430, emerald/anything). На окне 430 это
              ≈39 px, втрое больше плеча уголка.

              Лечится первым из двух выходов, которые ADR и называет: МЕТКИ
              ДЫШАТ ВМЕСТЕ С КАДРОМ. Две обёртки повторяют покой кадра ровно
              теми же переменными — своих чисел здесь нет и быть не может:
              `--lead-rest` тот же, что у .camera, а дыхание — ТОТ ЖЕ класс
              .drift, поэтому за ним автоматически едут и срез размаха вблизи
              (.zoomed .drift), и `prefers-reduced-motion`. Точка вращения у
              обоих слоёв общая — контрактные 50% 50% (CAMERA_ORIGIN).

              ПОБОЧНОЕ СЛЕДСТВИЕ НАЗВАНО ВСЛУХ: вместе с геометрией на те же
              ~12% растут подписи зон, искры и плечи уголков — всё, что задано
              в экранных px. Это цена регистрации, и она согласована с тем, как
              показан сам кадр: комната видна в 112%, значит и то, что лежит на
              ней, тоже. Вопрос дизайну задан письмом 63; счесть цену высокой —
              значит вернуть противомасштаб подписи, а не промах в 39 px. */}
          <div className={s.hotspotsRest}>
            <div ref={hotspotsFrameRef} className={s.drift}>
        ...
        {/* ОБЕЩАНИЕ ПУСТОЙ КОМНАТЫ — В КАДРЕ (тикет 208, пакет 47).
            «„Свет включится, когда появятся вещи" обязана быть в кадре — она
            обещание про комнату, а в полосе под ним становится подписью к
            кнопке». Доска рисует её ровно здесь: турн 51d (375×667) — 12 px от
            нижней кромки кадра, турн 41a (430×932) — 14, пилюля по центру.

            ЭТО ЖЕ ЗАКРЫВАЕТ РАСХОЖДЕНИЕ ФОРМУЛ. Пакет 44 мерил блок первого
            шага от кромки кадра с отступом 22, а подпись клал в кадр; у нас она
            стояла в полосе, и над блоком набегало 72 (в пустой комнате 53).
            Уехав в кадр, она забрала с собой и зазор под себя — строка действий
            пустой комнаты больше не резервирует ничего (globals.css →
            .imm-row-bare).

            ПОСЛЕДНЕЙ ВО ВЬЮПОРТЕ — значит поверх мест-уголков: порядок слоёв
            контракта (`places.json → visual.layer`) кончается подписью сцены, и
            держится он построением, а не z-index. Гаснет при наезде вместе с
            остальными оверлеями окна: в зоне на кадре стоит лист вещей, и
            обещание про пустую комнату там не к месту. */}
        {empty && (
          <div
            className={zoneOpen ? `${s.promise} ${s.promiseHidden}` : s.promise}
            aria-hidden
          >
            <span className={s.promisePill}>{t("emptyRoom")}</span>
          </div>
        )}
      </div>
      ...
      {/* Подсказка «коснись зоны» — ВНЕ вьюпорта (приёмка тикета 52): внутри
          ...
          В ПУСТОЙ КОМНАТЕ ЕЁ НЕТ ВОВСЕ (тикет 208): «коснись зоны» там врёт —
          касаться нечего, — а обещание, которое стояло здесь вместо неё, уехало
          в кадр, где ему и место. Пустой жёлоб под кадром — не потеря: место в
          нём забрал блок первого шага. */}
      {!empty && (
      ...
      {/* Нить-позиция (тикет 102, доска 28a): короткая дорожка по центру и
          сегмент на ней — «столько кадра ты видишь, и вот где ты в нём»
          (во всю ширину она наезжала на «поделиться» — тикет 122). Движение
