...
/**
 * Нить-позиция под сценой (тикет 102, доска 28a · `task15.json →
 * positionThread`): сегмент на дорожке показывает, какую часть кадра видно и
 * где она в нём. Пан говорит «комната шире окна» движением, нить — числом.
 *
 * Возвращает ДОЛИ, а не пиксели, и это важно: дорожка тянется по ширине
 * экрана (у неё только отступы по краям), а её пиксельную ширину знает
 * только браузер. Доли раскладываются в CSS через calc — мерить дорожку из
 * JS не нужно вовсе.
 *
 * - `segmentPct` — сколько процентов дорожки занимает сегмент: окно ÷ кадр
 *   (430 ÷ 630 ≈ 68.25%);
 * - `at` — положение сегмента: 0 у левого края кадра, 1 у правого. Сдвиг в
 *   пикселях считает уже вызывающий: ширину дорожки знает только браузер.
 *
 * ЧИСЛА СЧИТАЮТСЯ ПО НАШЕЙ ГЕОМЕТРИИ, а не берутся из письма: спецификация
 * даёт «кадр 1120, окно 430 → сегмент 148px», но 1120 — ширина ДЕСКТОПНОЙ
 * сцены из её же `rooms.json` (разбор задания 15). На телефоне кадр — 630
 * (ADR-0006), и сегмент выходит вдвое длиннее обещанного.
 */
export function phoneThread(pan: number): { segmentPct: number; at: number } {
  const { min, max } = phonePanRange();
  const travel = max - min;
  return {
    segmentPct: round4((scene.phone.w / scene.phone.image.w) * 100),
    at: travel === 0 ? 0 : round4((clampPan(pan) - min) / travel),
  };
}
