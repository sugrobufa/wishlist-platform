    ...
    <Section overline={t("lightOverline")}>
      ...
      <div ref={knobsRef} className={studio.knobs}>
        ...
        {/* ПОСЛЕДНЯЯ РУЧКА БЛОКА: её низ и есть та кромка, по которой кадр
            ...
            ДВА ПОЛОЖЕНИЯ, А НЕ ТРИ (тикет 256, приёмка владельца 16.08.2026):
            «свечной» упразднён — на комнате он не отличался от тёплого, а два
            одинаковых положения с разными именами это ложь интерфейсу. Ряд
            берёт их из LIGHT_COLORS, а не из головы, и колонок в нём столько,
            сколько положений: пустой клетке на месте снятого взяться неоткуда. */}
        <div className={studio.knobRow}>
