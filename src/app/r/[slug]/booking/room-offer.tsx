...
export function RoomOffer({ guestName, guestEmail, accent }: RoomOfferProps) {
  ...
  const fieldId = useId();
  // Праздник — это БУДУЩЕЕ: от даты просыпаются напоминания и «что подарили».
  // Год рождения в этом поле сделал бы комнату с датой, которая давно прошла,
  // поэтому браузеру говорим нижнюю границу. Считается на клиенте — компонент
  // и живёт только там, расхождения с сервером быть не может.
