...
export default async function GuestRoomPage({ params }: Params) {
  ...
  // звать гостя в пустую комнату-в-комнате незачем. Считает СЕРВИС
  // (services/guest-room → hasHall): после тикета 124 вещи сокровищницы в
  // комнату гостя не приезжают, и вывести это из отданных вещей больше
  // нельзя. Глазок хозяйки (тикет 89) там учтён.
  const hasHall = room.hasHall;
  ...
  return (
      ...
      style={
        {
          ...
          // У ГОСТЯ ПОД ПОВЕРХНОСТЬЮ НИЧЕГО НЕТ (тикет 143). Постоянный
          // таб-бар — экран хозяйки: гость не вошёл, вкладкам некуда вести.
    ...
    >
      ...
      <GuestBookingProvider slug={slug}>
        ...
        {/* НИЖНИЙ БАР ГОСТЯ (тикет 247). Вне указателя зон — он не про зоны, он
            про то, где человек находится и куда может уйти. А вот в канале
            «занято» он теперь нуждается (тикет 253): третье место спрашивает у
            него «есть ли у зрителя своя комната», чтобы вести домой, а не в
            регистрацию. Провайдер здесь тот же самый — второй завёл бы второй
            запрос канала на тот же экран. Разметки провайдер не добавляет
            (контекст без узла), поэтому бар как стоял поверх экрана (fixed),
            так и стоит. */}
        <GuestBar
