...
export default async function RoomPage() {
  ...
  /**
   * СКОЛЬКО МЕСТ В КОМНАТЕ — надстрочная пустого блока «Комната обставляется ·
   * N мест» (тикет 199, `round44/empty-room.json` → order[1]).
   *
        ...
        <div className="imm-rail imm-rail-bottom">
              ...
              emptyRoom ? (
                <div className="imm-empty-slot">
                  {/* ЗАГОЛОВОК И ОДНА ПОЛОСА СВЕТА (макет 41a). Одно действие,
                      ...
                      ПОЛОСА СВЕТА ГОВОРИТ «ДОБАВИТЬ ПЕРВУЮ ВЕЩЬ» (тикет 190,
                      та же приёмка): доска писала здесь «Вставить ссылку из
                      магазина», и владелец назвал это лишней активностью —
                      «там нужна просто кнопка добавить свою первую вещь…
                      простое и понятное действие, которое онбордит клиента».
                      Он прав по шагам: человеку с пустой комнатой и пустым
                      буфером надо уйти в магазин, найти вещь, скопировать
                      адрес и вернуться — три шага чужими руками до того, как
                      он увидит, ради чего всё.

                      ПАРСЕР ПРИ ЭТОМ НЕ СПРЯТАН: ссылка стоит ПЕРВЫМ полем
                      формы добавления, где ей и место — рядом с названием,
                      фото и зоной, и любое из них заполняется руками. Тот же
                      путь на одно нажатие длиннее, и он не требует ссылки от
                      того, у кого её нет. Адрес кнопки не менялся: /room/add
                      был здесь и до тикета — менялось только слово. */}
                  <div className="imm-empty-start">
                    {/* НАДСТРОЧНАЯ «КОМНАТА ОБСТАВЛЯЕТСЯ · N МЕСТ» (тикет 199,
                        `round44/empty-room.json` → order[1]).

                        ПРИГЛАШЕНИЕ ДЕЛАЕТ СЦЕНА, А НЕ КНОПКА. Мы спросили
                        дизайн, как выглядит пустая комната, когда действие
                        одно: одинокая кнопка посреди пустой полосы читается
                        заглушкой. Ответ — лечится не второй кнопкой, а
                        переносом веса наверх: тёмный кадр, вуаль, места-уголки
                        и обещание «свет включится, когда появятся вещи» уже
                        стоят, а полосе остаётся быть концом фразы. Эта строка
                        и есть её начало: она говорит, что комната не пуста, а
                        ждёт, и называет размер ожидания числом.

                        ЧИСЛО ЖИВОЕ (`emptyPlaces` выше): выключенная полка
                        исчезает вместе с мебелью, и «13» соврало бы всякому,
                        кто выключил хоть одну зону.

                        ПОСЛЕ КНОПКИ НЕ СТОИТ НИЧЕГО, и это то же правило с
                        другой стороны: заглушкой кнопка читается, когда под ней
                        остаётся пустое место. Здесь она последняя — пустого
                        «после» не существует. */}
                    <p className="overline imm-empty-places text-text-muted">
                      {t("emptyPlaces", { count: emptyPlaces })}
                    </p>
          ...
          >
            {/* Вход в добавление вещи (полировка 16). «Полоса света» — главная
                ...
                «Добавить первую вещь» в слоте `below` (тикет 190), и она видна
