 ...
 *
 * ЗДЕСЬ ЖИВЁТ ПОЛКА БЕЗ МЕСТА НА КАДРЕ (тикет 235, решение владельца 14.08.2026).
 * Раньше такая зона отвечала 404 наравне с выключенной, а её вещи уезжали в
 * «Что угодно»; теперь страница работает целиком — сетка, добавление, действия,
 * — и разницы с обычной полкой на этом экране нет ни в чём. Единственное, чего
 * у неё нет, живёт не здесь, а в кадре: метки и наезда камеры. Отсюда же
 * «полка N из M» у неё пуста — номер считается по меткам кадра (`zonePosition`).
 *
 * 404 ОСТАЁТСЯ РОВНО ЗА ДВУМЯ: за полкой, выключенной хозяйкой (`zonesOff`,
 * инвариант №5), и за адресом, скрытым продуктом (`zonesHiddenByProduct`) —
 * там зоны нет вместе с мебелью. Обе отсеивает `visibleZones` над пресетом,
 * который сам уже без скрытых адресов.
 */
export default async function ZoneListPage({ params }: Params) {
  ...
  // «полка 02 из 13» — место зоны среди меток кадра (тикет 99). У полки без
  // места на кадре номера нет вовсе (тикет 235): считать его не по чему, и
  // подпись пустой зоны обходится без него — `position` необязателен.
    ...
    <main className="min-h-screen pb-16">
      <div className="mx-auto w-full max-w-3xl px-5 lg:px-0">
        ...
        {/* Сетка хозяйки со слотом действий (тикет 13): «Спрятать/Показать»
            ...
            демо-вещей у неё не будет, а пустые вкладки рядом с карточкой
            копилки — шум (тикет 44). */}
        {items.length === 0 && zone.key !== MONEY_ZONE_KEY ? (
          ...
          // цели ниже (с тикета 192 она идёт после действий зоны), а не полка
          // без вещей.
          <EmptyZone
            zoneKey={zone.key}
        ...
        {/* Копилка на мечту (тикет 44) — только в зоне «Просто деньги». Здесь
            ...
            СТОИТ ПОСЛЕ ДЕЙСТВИЙ ЗОНЫ, а не перед ними (тикет 192) — тем же
            правилом, что лист зоны в сцене (`scene/zone-panel.tsx`). Пока
            карточка шла первой, её строка действий («Назвать цель») упиралась
            в «+ Добавить вещь» без отступа, и стык читался наслоением. Порядок
            у обоих мест вызова карточки теперь один. */}
