...
// (тикет 13), переезд в сокровищницу и обратно (тикеты 10 и 124), правка и
// перенос между зонами (тикет 39). Экшены тонкие (CLAUDE.md): ownership, Zod и
...
export type ItemActionResult =
  | {
      error: "AUTH" | "NOT_FOUND" | "NOT_IN_HALL" | "ZONE_NOT_VISIBLE" | "VALIDATION" | "GENERIC";
    }
  | undefined;
...
/**
 * Правка вещи и перенос между зонами (тикет 39). Место вещи правкой не
 * меняется: ключа `inHall` в схемах сервиса нет вовсе — переезд бывает только
 * явным действием. Вещь с активной бронью правится молча и тем же путём:
 * сказать «занято» нельзя (инвариант №1) — решение и объяснение живут в
 * комментарии к updateItem.
 */
...
/**
 * «В сокровищницу» (on=true) и «Вернуть в комнату» (on=false) — тикет 124.
 * Доступно у ЛЮБОЙ вещи и ведёт себя одинаково независимо от того, занята
 * она или нет: бронь снимается молча (инвариант №1, разбор — в toggleHall).
 */
