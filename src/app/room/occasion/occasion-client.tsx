          ...
          <li key={gift.itemId} className="flex items-center gap-3 bg-surface-app-ground py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {/* «СКАЗАТЬ СПАСИБО» — БЛАГОДАРНОСТЬ АДРЕСНАЯ, В СТРОКЕ ПОДАРКА
                  ...
                  ЧТО СТРОКА ДЕЛАЕТ, КОНТРАКТ НЕ ГОВОРИТ (спрошено письмом 54).
                  До ответа — `mailto:` на почту гостя; окажется другое, поменяется
                  ровно этот обработчик. Флаг и адрес приходят порознь (сервис):
                  сегодня они ходят парой — одна бронь, одна почта, — а у
                  складчины строка положена, если почта есть хотя бы у одного. */}
              {gift.canThank && gift.thanksEmail !== null && (
                <a
                  href={`mailto:${gift.thanksEmail}`}
