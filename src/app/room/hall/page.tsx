  ...
  return (
    ...
    <main className="min-h-[100dvh] bg-surface-hall-ground pb-[calc(var(--imm-tab-bar)+30px)]">
      ...
      <div className="mx-auto w-full max-w-3xl px-5 pt-8 lg:px-0">
        {/* Чем это место отличается от комнаты — одной тихой строкой (тикет
            214, приёмка владельца 13.08.2026: «нужен небольшой поясняющий
            текст, что тут хранятся вещи которыми ты гордишься и уже
            приобретены тобой»). Гостю дизайн такую строку дал
            (`guestSubtitle`), хозяйке — нет.

            ТОЛЬКО ПРИ НЕПУСТОЙ ВИТРИНЕ. У пустой это место занято `Hall.empty`,
            где сказано то же самое и названы три дороги сюда: две
            объяснительные строки подряд читаются уже инструкцией. */}
        {items.length > 0 && (
          <p className="mb-5 max-w-md text-sm leading-relaxed text-text-muted">
            {t("ownerSubtitle")}
          </p>
        )}
