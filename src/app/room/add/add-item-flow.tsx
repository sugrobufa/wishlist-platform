...
export function AddItemFlow({
  ...
  function buildInput(photoKey: string | undefined) {
    ...
    return {
      // «Уже моё» после отмены состояний означает СОКРОВИЩНИЦУ, и только её
      // (тикет 124). Раньше вещь ложилась в зону вещью «люблю», а флаг
      // витрины ставился отдельно (`?hall=1`, тикет 89) — теперь оба входа
      // ведут в одно место, и второй дороги нет.
      //
      // ВРЕМЕННО. Сам выбор «люблю \ хочу» на этом экране отменён и должен
      // исчезнуть — это работа захода про экраны (переключателя быть не
      // должно вовсе). Пока он жив, его ответ переводится в место честно,
      // чтобы форма не падала валидацией и не заводила вещь не туда.
