...
/**
 * Сколько это в ЦЕЛЫХ неделях — или null, если нацело не делится (тикет 210,
 * `round47/strings-delta.json` → `asked.days`).
 *
 * ЗАЧЕМ. Плашка предложения приходит РОВНО за три недели (`OFFER_LEAD_DAYS`),
 * и в первый свой день говорила «Через 21 день Новый год». Дизайн прислал
 * второй ключ отсчёта со словами: «предложение приходит именно за три недели, и
 * „через 21 день" там читается хуже». Правило вышло одно на все числа окна:
 * 21 → три недели, 14 → две, 7 → одна, 20 → двадцать дней.
 *
 * АРИФМЕТИКА ЗДЕСЬ, А СЛОВО — В ПЛАШКЕ: календарь отвечает «делится ли», выбор
 * между `Occasion.leftWeeks` и `Occasion.leftDays` делает та, кто печатает
 * строку (`components/occasion/occasion-offer`). Разводить их незачем и нельзя
 * в другую сторону: словарь календарь не видит.
 */
export function wholeWeeks(daysLeft: number): number | null {
  return daysLeft > 0 && daysLeft % 7 === 0 ? daysLeft / 7 : null;
}
