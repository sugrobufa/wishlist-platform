...
/**
 * Форма правки вещи КОМНАТЫ: цена, размеры, степень желания, впечатление и
 * ССЫЛКА (тикет 244, приёмка владельца 14.08.2026: «не вижу, как добавить
 * возможные точки продаж данного подарка»).
 *
 * До этого тикета ссылка была свойством РОЖДЕНИЯ вещи: её принимало только
 * создание, и вещь, заведённая руками, оставалась без магазина навсегда, а
 * ошибочную ссылку из парсера нельзя было поправить — только удалить вещь и
 * завести заново. Блок «Где купить» при этом собран с тикета 37 и работает —
 * ему нечего было показывать.
 *
 * Пустая строка здесь — «стереть», а не «не менять»: поле формы отправляется
 * всегда, и молчаливое сохранение прежней ссылки означало бы, что стереть её
 * нельзя ничем. `source` при этом не трогается — он история появления вещи, а
 * не наличие у неё магазина.
 */
  ...
  const changed = await prisma.item.updateMany({
    ...
    data:
      "price" in data
        ? {
            ...common,
            // Ссылка пишется как дана, производные считает СЕРВЕР (тикет 195):
            // клиент canonicalUrl/domain не отдаёт и отдать не может.
            url: data.url ?? null,
            ...urlMetaFor(data),
            price: new Prisma.Decimal(data.price),
            currency: data.currency,
