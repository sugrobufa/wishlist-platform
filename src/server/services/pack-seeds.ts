// Ядро «положить курируемый набор в комнату» — ИНСТРУМЕНТ РАЗРАБОТКИ, посев
// стенда (тикет 61, `npm run db:seed`).
//
// ПОТРЕБИТЕЛЬ ОСТАЛСЯ ОДИН (тикет 191). Вторым было «Или начни с готового» —
// предложение живому человеку наполнить комнату одной кнопкой. Владелец снял
// его 11.08.2026: «для любой женщины это оскорбление, ведь она сама должна
// наполнить пространство». Модуль пережил снятие целиком и намеренно: стенд для
// того и стоит, чтобы показывать продукт владельцу, а предложением человеку он
// не был никогда. Дорога отсюда в живую комнату — только через `db:seed`.
//
...
//     tokens.json (CLAUDE.md: расхождение с пакетом = баг). До тикета 175 стенд
//     сеялся своим набором, и владелец смотрел на вещи, которые придумали мы,
//     — теперь он смотрит на вещи пакета (`livePoolSeeds`, `standPoolSeeds`);
//
...
//
// ПОЛОВИНЫ НЕ ПОВТОРЯЮТ ДРУГ ДРУГА (тикет 183). Источника два, а вещи они
// называют одинаково, и на стенде вышло сто пар «хочу то, что у меня уже
// есть»: контракт хочет «Букет пионов», демо-пул кладёт его же на витрину.
// Желание, которое в той же зоне уже стоит своей вещью, посев не заводит —
// `withoutOwnedWishes` ниже; зову его `standZoneSeeds` (services/stand-seed),
// потому что единица сравнения — ЗОНА, а не пул.
//
...
/**
 * Зёрна по пулам — контракт, пропущенный через сторожа. Считается один раз при
 * загрузке модуля (как `rooms` в config/design): посев ходит по всем зонам
 * комнаты, и разбирать ради каждой json заново незачем.
 */
...
/**
 * Желания контракта дизайна по ключу пула — в нём только желания и только с
 * ценой (см. сторожа выше).
 *
 * ИМЯ ОСТАЛОСЬ ОТ ДВУХ ПОТРЕБИТЕЛЕЙ, ПОТРЕБИТЕЛЬ ТЕПЕРЬ ОДИН (тикет 191):
 * «живыми» эти зёрна звались в отличие от витринных «уже своё», потому что их
 * получал и человек по кнопке «начни с готового». Кнопки больше нет — комнату
 * за человека не наполняют (решение владельца 11.08.2026), — и остался посев
 * стенда. Различие имён при этом живо: `livePoolSeeds` против `hallSeeds`.
 *
...
/**
 * Название для СРАВНЕНИЯ ДВУХ ПОЛОВИН: регистр и лишние пробелы, и ничего
 * больше (тикет 183). `\s` в JS ловит и неразрывный пробел, поэтому «Чемодан
 * 55 см» с NBSP из контракта сравнится с обычным пробелом демо-пула.
 *
 * Умнее нарочно не делаем: «Парфюм Nº7» и «Парфюм № 7» здесь разные вещи.
 * Всё, что дальше регистра и пробелов, — угадывание смысла, а ошибка
 * угадывания стоит дорого: желание пропадёт из комнаты молча.
 */
function sameThingKey(title: string): string {
  return title.replace(/\s+/g, " ").trim().toLowerCase();
}

/**
 * ЗЁРНА ЗОНЫ БЕЗ «ХОЧУ ТО, ЧТО ТУТ УЖЕ СВОЁ» (тикет 183).
 *
 * Половины посева приезжают из разных источников (желания — контракт дизайна,
 * «уже своё» — демо-пулы) и называют вещи одинаково. В базе это не дубль —
 * экраны разные, — но человеку комната говорит неправду: комната — чего
 * хочется, сокровищница — что уже моё (CLAUDE.md, решение владельца 09.08), и
 * «хочу букет пионов» при живом букете пионов на витрине не значит ничего.
 *
 * Снимается ЖЕЛАНИЕ, а не «уже своё»: сокровищница — факт, желание — намерение,
 * и спорит с фактом именно оно. Порядок половин при этом не меняется —
 * фильтр сохраняет исходную последовательность.
 *
 * Чинится ПОСЕВ, а не содержимое пулов: переименование в `demo-pools`
 * вылечило бы сегодняшние совпадения и молча пустило завтрашние — контракт
 * дизайна приезжает пулами по пять, и следующая партия принесёт свои.
 */
export function withoutOwnedWishes(seeds: readonly PackSeed[]): readonly PackSeed[] {
  const owned = new Set(seeds.filter((seed) => seed.mine).map((seed) => sameThingKey(seed.title)));
  if (owned.size === 0) return seeds;
  return seeds.filter((seed) => seed.mine || !owned.has(sameThingKey(seed.title)));
}
