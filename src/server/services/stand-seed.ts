 ...
 * (это же проверяет rooms.changeRoomPreset) и по замыслу пакета ловит всё, что
 * не влезло в категории. Добор — целый пул категории, которой в ЭТОМ интерьере
 * не досталось полки: это ровно то, что зона и означает.
 *
 * Полок «не досталось» с тикета 235 заметно меньше: зона без предмета в кадре
 * больше не исчезает, она живёт без места на кадре (`zonesWithoutRect`), и её
 * вещи в «Что угодно» не уезжают.
 */
