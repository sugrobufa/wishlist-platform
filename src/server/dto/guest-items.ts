...
// форме не было вовсе — гость выбирал подарок и упирался в тупик. У вещи
// комнаты есть ключ `shop` (домен + канонический адрес), и правило показа у
// него СВОЁ, не общее с ценой (тикет 195, решение владельца 11.08.2026):
// - **ссылка есть — ссылка показывается**, при любом `priceVisibility`.
//   «Сколько стоит» и «где взять» — разные вопросы: скрывая цену, хозяйка
//   прячет число в своей комнате, а не запрещает гостю купить вещь. До 195
//   ключ `shop` стоял внутри того же `if`, что price/currency, и вещь со
//   скрытой ценой становилась недостижимой — блок «Где купить» молча исчезал
//   и с плитки, и с гостевой карточки;
...
//   (parser/normalize: https, хост в нижнем регистре, трекинг-параметры
//   выброшены). Сырой `Item.url` — пользовательский ввод, и гостю он не
//   показывается ни адресом, ни доменом (инвариант №6).
 ...
 * прояснило. С тикета 195 разницы не осталось и в ПРАВИЛЕ ПОКАЗА: ссылка
 * видна и хозяйке, и гостю всегда, когда она есть.
 */
 ...
 * а ключ shop при этом остаётся: у ссылки своё правило (тикет 195, см. шапку
 * файла). Бронируется здесь ВСЁ (тикет 124): комната и есть список желаний.
 */
export type GuestRoomItemDto = GuestItemBaseDto & {
  ...
  /** Где купить. Ключ есть при разбираемом canonicalUrl — цена ни при чём (195). */
  shop?: GuestShopDto;
  size: string | null;
...
export function itemForGuest(item: Item): GuestItemDto {
  ...
  // ССЫЛКА — ВНЕ `if` ЦЕНЫ (тикет 195). Раньше она стояла внутри: страница
  // магазина показывает цену, и это считалось обходом настройки хозяйки.
  // Довод решал не ту задачу — скрытая цена прячет ЧИСЛО в комнате, а не путь
  // к вещи, и продукт существует ровно затем, чтобы гость её купил
  // (инвариант №8 в этой формулировке и записан). Единственное условие теперь
  // одно: адрес есть и разбирается.
