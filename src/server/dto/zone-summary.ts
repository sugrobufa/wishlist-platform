...
// «Свободно» — число, ПРОИЗВОДНОЕ от броней, и в ветке хозяйки его нет:
// «7 свободно» при восьми положенных вещах говорит ей ровно то же, что
// «забрали одну из восьми». Числа хозяйки (`count`, `wantCount`, `more`)
// считаются ТОЛЬКО по её собственным вещам и от броней не зависят вообще —
// закреплено тестом (tests/zone-summary.dto.test.ts, правило 1). Появится
// когда-нибудь «свободно» для гостя — оно обязано родиться в гостевой ветке
// и на входе получить брони, которых в этой форме сегодня нет ни у кого.
//
 ...
 * ни одного числа. Всё, что она здесь получает (`count`, `wantCount`, `more`,
 * миниатюры), — её собственные данные о СВОИХ вещах; броней эта функция не
 * видит по построению, потому что на вход им приехать неоткуда.
 */
