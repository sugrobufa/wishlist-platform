...
// марок»; переформулировано тикетом 124). Три условия, и все три обязательны:
//   1. считать только по вещам КОМНАТЫ, чью цену ЭТОТ зритель имеет право
//      видеть;
...
// вещи, и правило про цены без правила про марки не работает. Пороги (три и
// три) тикет 124 не менял.
// ВЕЩЬ СОКРОВИЩНИЦЫ В АГРЕГАТ НЕ ВХОДИТ НИКОГДА. Раньше это правило звучало
// «вещь „люблю" не входит»: она уже подарена, её незачем оценивать. Теперь
// то же самое сказано местом — витрина не участвует в разговоре о подарках, и
// цены у неё в форме нет в принципе (инвариант №8).
//
// СЧЁТЧИКИ (тикет 124, модель v2 дизайна):
//   всего вещей — хозяйке и гостю, ОДНО число («4 вещи»);
//   «M свободно» — ТОЛЬКО гостю и НЕ ОТСЮДА (см. ниже);
//   СКОЛЬКО ЗАБРАЛИ — НИКОГДА и никому, кроме шапки комнаты хозяйки.
// Числа «помечено „хочу"» больше нет: в комнате всё — желание, и оно всегда
// равнялось бы `count`.
// Ключа про брони в этой форме не существует вовсе — ни в какой её
...
//
// ОТСЮДА ЖЕ — ответ дизайна раунда 6 (тикет 41), который просили записать
// словами, а не оставить «оно и так не считается»:
//   гостю   — занято и свободно по зоне: это его координация, ради неё мы и
//             спрашиваем у него имя;
//   хозяйке — про занятость по зонам НИЧЕГО; один счётчик на комнату.
// «Свободно» — число, ПРОИЗВОДНОЕ от броней, и в этой форме его нет ни у
// кого, включая гостя. Причина не в стеснительности, а в кэше: сводка
// складывается внутри кэша комнаты (services/guest-room), а хозяйка открывает
// СВОЮ ЖЕ ссылку и получает тот же кэшированный HTML — «7 свободно» при
// восьми вещах сказало бы ей ровно то же, что «забрали одну из этой полки»
// (инвариант №1: «ни в API, ни в кэше»). Поэтому «M свободно» гостю считает
// экран, вычитая из `count` занятые вещи зоны из НЕкэшируемого канала
// «занято» (тикет 08) — а хозяйке тот же канал отвечает пустотой
// (services/bookings.takenForOwner), и число у неё не появляется вовсе.
// Числа хозяйки (`count`, `more`, миниатюры) считаются ТОЛЬКО по её
// собственным вещам и от броней не зависят вообще — закреплено тестом
// (tests/zone-summary.dto.test.ts, правило 1).
//
...
/**
 * Миниатюра — только фотография. Признака состояния здесь больше нет
 * (тикет 124): он существовал ради пунктира, кодировавшего «хочу»
 * (инвариант №3), а инвариант отменён целиком — в комнате все вещи одинаковы,
 * плитка у всех с обычной границей.
 */
...
export type ZoneSummaryDto = {
  ...
  zone: string;
  /** Всего вещей КОМНАТЫ — без спрятанных, без витринных и без призраков. */
  count: number;
 ...
 */
type SummarySource = {
  /** Место вещи: витринные в сводку зоны не входят ни одним числом. */
  inHall: boolean;
  photoUrl: string | null;
  isDemo: boolean;
  hidden?: boolean;
...
/**
 * Вещи, по которым СЧИТАЮТСЯ ОБА агрегата: вещь КОМНАТЫ + ключ price
 * ПРИСУТСТВУЕТ. Ровно в этом ключе и живёт разница между хозяйкой и гостем —
 * форма вещи уже решила её за нас (тикет 07): у гостя при priceVisibility
 * ME/NONE ключа нет, у витринной вещи его нет ни у кого. Второй раз ту же
 * развилку мы не изобретаем.
 */
function priced(
...
export function emptyZoneSummary(zoneKey: string): ZoneSummaryDto {
  return { zone: zoneKey, count: 0, thumbs: [], more: 0 };
}
...
function summarize(zoneKey: string, items: readonly SummarySource[]): ZoneSummaryDto {
  // Спрятанное, витринное и примеры — вон, до всех подсчётов. Сводка зоны
  // описывает КОМНАТУ: вещь, уехавшая в сокровищницу, не считается ни в
  // счётчике, ни в миниатюрах, ни в агрегатах (тикет 124).
  const visible = items.filter(
    (item) => item.hidden !== true && !item.isDemo && !item.inHall,
  );
...
/**
 * Сводка зоны для хозяйки: её собственные цены видны ей все, а про брони —
 * ни одного числа. Всё, что она здесь получает (`count`, `more`, миниатюры),
 * — её собственные данные о СВОИХ вещах; броней эта функция не видит по
 * построению, потому что на вход им приехать неоткуда.
 */
