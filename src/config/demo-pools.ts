...
//
// ЭТО БОЛЬШЕ НЕ СТАРТОВЫЙ НАБОР. С тикета 136 зёрна «начни с готового»
// приезжают КОНТРАКТОМ ДИЗАЙНА (`handoff/seeds/seeds.json`, 19 пулов по 5
// желаний) и читаются в `src/server/services/pack-seeds.ts`. Здесь остались
// два потребителя, которым контракт не подходит: призраки (форма owner-DTO)
// и посев стенда — стенду нужны вещи «уже своё» с дарителем и годом, иначе
// витрине сокровищницы нечего показывать, а живому человеку такое семя не
// достаётся никогда (инвариант №2). Новые зёрна набора — в контракт, не сюда.
//
