...
export type RoomZone = {
  ...
  rectOld?: ZoneRect;
  /**
   * НЕ ЧИТАТЬ КАК «кадр подключён». Флаг означает «прошёл порог приёмки раунда
   * 3», и после двух переразметок он устарел: раунд 5 оставил `accepted: true`
   * на шести зонах, чей прямоугольник в том же раунде сдвинулся, — порог им
   * мерили по другому участку кадра. Сам контракт с ними не согласен: у всех
   * шести `openFrame: null`.
   *
   * Поэтому единственный источник истины про раскрытие — `openFrame`, и код
   * смотрит только на него: 33 зоны с кадром против 39 с флагом. Тест
   * `design-contract` держит это расхождение поимённо, чтобы следующий пакет
   * либо его починил, либо объяснил.
   */
  accepted?: boolean;
  /** Кадр «открыто» порог не прошёл и ждёт пересъёмки (openFrame: null). */
  reshoot?: boolean;
