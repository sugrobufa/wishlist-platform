...
export type RoomZone = {
   ...
   *
   * ДЕСЯТЬ — ЧИСЛО ОКОНЧАТЕЛЬНОЕ (ADR-0009, 07.08.2026). Съёмка раскрытий
   * остановлена: за три раунда и ~170 снимков подключилось 10 из 130, а самая
   * полная комната закрыта на 4 зоны из 13, и все оставшиеся — того типа,
   * который правило съёмки само объявляет неснимаемым. Остальным зонам даёт
   * жизнь отклик светом (тикет 64). Ждать одиннадцатого кадра не нужно.
   */
  accepted?: boolean;
  /**
   * Кадр «открыто» порог не прошёл (openFrame: null). ЭТО ЛЕТОПИСЬ, А НЕ
   * ОЧЕРЕДЬ (ADR-0009): 112 зон несут флаг, и ни одна из них пересъёмки не
   * ждёт. Код на него не смотрит вовсе — решает только `openFrame`.
   */
  reshoot?: boolean;
