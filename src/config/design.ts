 ...
 *
 * ЭТОТ СПИСОК БОЛЬШЕ НЕ ЗНАЧИТ «ПРЕДМЕТА НЕТ В ИНТЕРЬЕРЕ» (тикет 235). До
 * 14.08.2026 в нём стояли все восемь адресов без предмета, и каждый отвечал
 * 404: зона исчезала целиком, а её вещи уезжали в «Что угодно». Владелец это
 * отменил — зона без предмета осталась ЖИВОЙ полкой, у которой просто нет места
 * на кадре (`zonesWithoutRect` ниже). Здесь остались только те, кто **ждёт
 * прямоугольника**: предмет им дизайн нашёл пакетом 50, а измерить его — работа
 * тикета 234. Приедут координаты — строки уйдут отсюда по одной, и список
 * станет пустым и рабочим, как `zoneKeysHiddenByProduct` выше.
 *
 *   warm/music    — проигрыватель на мраморной консоли справа (пакет 50);
 *   lux/music     — проигрыватель в мраморной нише справа, полкой ниже книг;
 *   sport/watches — витрина с часами на полке справа;
 *   study/tech    — плёночная камера на кожаном сундуке слева внизу.
 *
 * ПОЧЕМУ ДВЕРЬ ОСТАЁТСЯ. Это единственный способ сказать «этой зоны в этой
 * комнате нет» одной строкой, а не новым механизмом. 404 за ней — та же 404,
 * что за `Room.zonesOff`: зоны нет вместе с мебелью (инвариант №5).
 *
 * ЧТО С ВЕЩАМИ ВНУТРИ. Вещи в базе остаются: `Item.zone` — свободная строка без
 * внешнего ключа. Но смена интерьера уводит их в «Что угодно»
 * (`rooms.changeRoomPreset`), и это ровно та потеря полки, ради которой тикет
 * 235 и завёл третье состояние. Разбор — ADR-0006, «Вещи в скрытой зоне».
 */
 ...
 *
 * ТРИ АДРЕСА `bar` ЗАПИСАНЫ ВПЕРЁД: самой зоны в `rooms.json` ещё нет, её
 * заводит тикет 234 вместе с прямоугольниками. Строка стоит здесь заранее
 * нарочно — иначе новая полка появится в кадре меткой на «прямоугольнике по
 * месту» и соврёт в первый же день. Тест называет такие адреса поимённо, чтобы
 * они не копились молча.
 */
