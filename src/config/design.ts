...
export type RoomZone = {
   ...
   *
   * Поэтому с приёмки 46 снова выполняется `accepted ⟺ openFrame` — 30 зон, и
   * тест `design-contract` это правило сторожит. Единственный источник истины
   * про раскрытие по-прежнему `openFrame`: код смотрит только на него.
   */
  accepted?: boolean;
