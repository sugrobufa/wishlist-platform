...
test("комната гостя: таб-бара нет, и пустоты под него тоже нет", async ({ page }) => {
  ...
  // …вторая половина правила, теперь В НОВЫХ ГРАНИЦАХ. Смысл тикета 143 —
  // «пустоты под баром не бывает», — и до 14.08 он означал «поверхность доходит
  // до низа окна»: бара у гостя не было вовсе. Теперь бар есть, его 86 px
  // законны, и то же правило читается иначе: поверхность кончается ровно там,
  // где бар начинается. Пустота вернулась бы разрывом между этими двумя
  // числами, и он бы здесь и покраснел.
  const barRect = await rectOf(guestBar, "гостевой бар");
  expectSame(surface.bottom, barRect.top, "низ поверхности гостя против верха его бара");
  expectSame(barRect.bottom, viewport, "низ гостевого бара против низа окна");
