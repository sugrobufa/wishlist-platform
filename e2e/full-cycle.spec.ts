...
test("полный цикл дарения: хозяйка → гость → счётчик → «что подарили» → зал славы и связи", async ({
  ...
  await test.step("итог вручную → строка с именем → «Дошло» → «уже в зале славы»", async () => {
    ...
    // состоянии «дня рождения нет» (тикет 216) — заголовок тот же, а ручное
    // закрытие стоит ТИХОЙ строкой, а не полосой света (тикет 217): гореть
    // между праздниками громкой кнопке владелец запретил.
    await expect(hostessPage.getByRole("heading", { name: "Праздник ещё впереди" })).toBeVisible();
    await hostessPage.getByRole("button", { name: /подвести итог/ }).click();
